...
class Rekapdatawp extends MY_Controller
{
    ...
    public function ajax_list()
    {
        ...
        set_time_limit(3600);
        $stat_arr = array(
            "1" => "<i class='fas fa-circle' style='color:green;'></i>",
            "2" => "<i class='fas fa-circle' style='color:red;'></i>"
        );
        ...
        foreach ($list as $pel) {
            $no++;
            $npwpd = '32.75.'.$pel->camat_kode.'.'.$pel->lurah_kode.'.'.$pel->wp_wr_no_urut.'.'.$pel->wp_wr_gol.'.'.$pel->ref_kodus_kode;
            $row = array();
            $row[] = $no;
            // $row[] = $pel->npwpd;
            $row[] = $npwpd;
