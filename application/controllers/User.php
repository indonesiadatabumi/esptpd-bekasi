...
class User extends MY_Controller
{
    ...
    public function insert()
    {
        ...
        if ($cek->num_rows() > 0) {
            ...
            if ($this->input->post('nip')) {
                $npwp_nik = $this->input->post('nip');
            } else {
                $npwp_nik = $username;
            }

            $no_reg = $this->Mod_user->get_register_user();
            ...
            $this->Mod_user->insertUser("tbl_users", $save);
            echo json_encode(array("status" => TRUE));
        }
    }
        ...
        if (!empty($_FILES['imagefile']['name'])) {
            ...
            if ($this->upload->do_upload('imagefile')) {
                ...
                if ($this->input->post('password')) {
                    $save  = array(
                        'npwpd' => $this->input->post('username'),
                        'nama' => $this->input->post('full_name'),
                        'email' => $this->input->post('email'),
                        'password'  => md5($this->input->post('password')),
                        'id_level'  => $this->input->post('level'),
    ...
    public function delete()
    {
        ...
        $this->Mod_user->deleteUsers($id, 'tbl_users');
        $data['status'] = TRUE;
        echo json_encode($data);
    }
    ...
    function get_namapegawai()
    {
        $nip = $this->input->post('npwpd');

        $result = $this->Mod_user->get_namapegawai($nip);
        // $result = ($result != '') ? $result : 'NIP salah /Tidak Terdaftar';
        echo json_encode($result);
    }
