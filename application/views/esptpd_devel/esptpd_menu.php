                ...
                <table class="table table-striped  table-bordered">
                    ...
                    <tbody>
                        ...
                        foreach ($esptpd as $row) {
                            ...
                            //             </td>
                            //         </tr>";
                            $cek_masa_pajak = $Mod_esptpd_devel->cek_masa_terakhir($row->npwprd);
                            if ($cek_masa_pajak == null) {
                                echo "<tr><td> 1 </td>
                                        <td>" . $row->npwprd . "</td>
                                        <td>" . $row->wp_wr_nama . "</td>
                                        <td align='center'>
                                        <a href=" . base_url('esptpd_devel/billing_new/') . $row->wp_wr_bidang_usaha . "/" . $row->wp_wr_no_urut . " class='btn btn-sm btn-outline-success _lapor'><i class='fa fa-info'></i> Detail </a>&nbsp;
                                        <button   onclick=\"window.location='" . base_url('esptpd_devel/add/') . $row->wp_wr_bidang_usaha . "/" . $row->wp_wr_no_urut . "'\"  class='btn btn-sm btn-outline-info _lapor'><i class='fa fa-edit'></i> Generate Billing</button>
                                        </td>
                                    </tr>";
                            }else{
                                $cek_lapor = $Mod_esptpd_devel->cek_lapor($cek_masa_pajak->spt_periode_jual1, $cek_masa_pajak->spt_id);
                                $wp_catering = substr($row->npwprd, 4, -14);
