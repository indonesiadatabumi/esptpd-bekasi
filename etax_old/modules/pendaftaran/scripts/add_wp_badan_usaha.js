var getNextNomor = function() {
	$.ajax({
		type : "GET",
		url : GLOBAL_WP_BU_VARS["get_next_number_wp"],
...
var createEventToolbar = function (){
	$("#btn_view").click(function (){
		// load content
		load_content(GLOBAL_WP_BU_VARS["view_wp_badan_usaha"]);
	});
	...
	var submitForm = function() {
		...
		var save_options = { 
			url : GLOBAL_WP_BU_VARS["save_wp_badan_usaha"],
			dataType: 'json',
		...
		$("#frm_add_wp_bu").ajaxSubmit(save_options);
	};
	...
	$("#btn_cetak_kartu").click(function() {
		if ($("input[name=wp_wr_id]").val().length > 0) {
 			//alert($("input[name=id]").val());
 			url = GLOBAL_WP_BU_VARS["cetak"] +
						"?wp_id=" + $("input[name=wp_wr_id]").val();
			...
			showWarning("Data tidak bisa dicetak.");
		}
	});
};
