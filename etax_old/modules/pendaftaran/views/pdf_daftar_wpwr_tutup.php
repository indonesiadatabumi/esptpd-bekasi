...
$pdf = new pdf_usage('L','mm','legal'); // Landscape dengan ukuran kertas A4
$pdf->Open();
...
$pdf->MultiCell(190,4,$pdf->SetFont('Arial','',6).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"Telp. " .
					...
					$pdf->Cell(190,4,"DAFTAR WP TUTUP ".strtoupper($bidus).$judulplus,'','','C').$pdf->Cell(48,4,"",'','','C'),'','','',0);
...
$pdf->MultiCell(190,4,$pdf->SetFont('Arial','',5).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"",'','','L').
					$pdf->SetFont('Arial','B',10).$pdf->Cell(190,4,"Dari tanggal $fDate s/d $tDate ",'','','C').
					$pdf->Cell(48,4,"",'','','C'),'','','',0);
...
$th1[3]['TEXT'] = "<b>PEMILIK/PENANGGUNG JAWAB</b>";
$th1[4]['TEXT'] = "<b>N P W P D</b>";
$th1[5]['TEXT'] = "<b>TANGGAL TUTUP</b>";
if ($this->input->get('bidus') == "") {
	$th1[6]['TEXT'] = "<b>JENIS PAJAK</b>";
	$th1[5]['BRD_TYPE'] = 'LRT';
	$th1[6]['BRD_TYPE'] = 'LRT';
} else {
	$th1[5]['BRD_TYPE'] = 'LRT';
...
if(!empty($arr_data)) {
	...
	foreach($arr_data as $k => $v) {
		...
		$data1[3]['TEXT'] = "" . $v['wp_wr_nama_milik'];
		$data1[4]['TEXT'] = "" . $v['npwprd'];
		$data1[5]['TEXT'] = format_tgl($v['wp_wr_tgl_tutup']);
		if ($this->input->get('bidus') == "") {
			$data1[6]['TEXT'] = strtoupper($v['ref_kodus_nama']);
		}
		$data1[0]['T_ALIGN'] = 'R';
		$data1[3]['T_ALIGN'] = 'C';
		$data1[4]['T_ALIGN'] = 'C';
		...
		if ($this->input->get('bidus') == "") {
			$data1[5]['BRD_TYPE'] = 'LT';
			$data1[6]['BRD_TYPE'] = 'LRT';
			$data1[6]['T_SIZE'] = 8;
			$data1[6]['LN_SIZE'] = $linespace;
		} else {
...
if ($this->input->get('mengetahui') != "0" && $this->input->get('pemeriksa') != "0") {
	...
	$ttd1[0]['TEXT'] = $mengetahui->ref_pangpej_ket.' ,'.$mengetahui->ref_goru_ket;
	$ttd1[1]['TEXT'] = $pemeriksa->ref_pangpej_ket.' ,'.$pemeriksa->ref_goru_ket;
	$pdf->tbDrawData($ttd1);
