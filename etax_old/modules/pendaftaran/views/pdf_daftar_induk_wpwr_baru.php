...
$pdf->MultiCell(198,3,$pdf->SetFont('Arial','B',10).$pdf->Cell(12,3,"",'','','C').
...
$pdf->MultiCell(198,3,$pdf->SetFont('Arial','',8).$pdf->Cell(12,3,"",'','','C').
					$pdf->Cell(50,3,ucwords(strtolower($pemda->dapemda_lokasi))." - ".
					ucwords(strtolower($pemda->dapemda_ibu_kota)),'','','L').$pdf->Cell(80,3,"",'','','C').
					$pdf->Cell(48,3,"",'','','C'),'','','',0);
					
$pdf->MultiCell(198,4,$pdf->SetFont('Arial','',8).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"Telp. " .
...
if (empty($fDate)) {
	$fDate = "Yang Terdaftar dari Tanggal";
}
else {
...
$pdf->MultiCell(198,4,$pdf->SetFont('Arial','',5).$pdf->Cell(12,4,"",'','','C').$pdf->Cell(50,3,"",'','','L').
					$pdf->SetFont('Arial','B',9).$pdf->Cell(190,4,"$fDate s/d $tDate ",'','','C').
					$pdf->Cell(192,4,"",'','','C'),'','','',0);

$pdf->MultiCell(198,4,"",'','','',0);
...
if ($_GET['camat'] != "")  { 
	...
	if($_GET['lurah'] == "") {
		$pdf->MultiCell(198,4,$pdf->SetFont('Arial','',8).$pdf->Cell(198,4,"".$kecamatan,'','','L'),'','','',0);
	}
	else {
		$lurah = explode("|", $_GET['lurah']);
		$kelurahan = "Kelurahan : " . $lurah[1];
		$pdf->MultiCell(198,4,$pdf->SetFont('Arial','',8).$pdf->Cell(95,4,"".$kecamatan,'','','L').$pdf->Cell(95,4,"".$kelurahan,'','','L'),'','','',0);
	}
}
...
$th1[3]['TEXT'] = "<b>PEMILIK</b>";
$th1[4]['TEXT'] = "<b>N P W P D</b>";
$th1[5]['TEXT'] = "<b>Tgl. DAFTAR</b>";
$th1[6]['TEXT'] = "<b>KETERANGAN</b>";
			...
			foreach($ar_data as $k => $v) {
				...
				if(!empty($ardat)) {
					...
					foreach($ardat as $k1 => $v1) {
						...
						$data2[3]['TEXT'] = $v1[wp_wr_nama_milik];
						$data2[4]['TEXT'] = $v1[npwprd];
						$data2[5]['TEXT'] = $v1[wp_wr_tgl_kartu];
						$data2[6]['TEXT'] = "";
						
						$data2[0]['T_ALIGN'] = 'R';$data2[3]['T_ALIGN'] = 'L';$data2[4]['T_ALIGN'] = 'C';$data2[5]['T_ALIGN'] = 'C';$data2[6]['T_ALIGN'] = 'C';
						$data2[0]['BRD_TYPE'] = 'LT';$data2[1]['BRD_TYPE'] = 'LT';$data2[2]['BRD_TYPE'] = 'LT';$data2[3]['BRD_TYPE'] = 'LT';;$data2[4]['BRD_TYPE'] = 'LT';;$data2[5]['BRD_TYPE'] = 'LT';$data2[6]['BRD_TYPE'] = 'LRT';
						$data2[0]['T_SIZE'] = 8;$data2[1]['T_SIZE'] = 8;$data2[2]['T_SIZE'] = 8;$data2[3]['T_SIZE'] = 8;$data2[4]['T_SIZE'] = 8;$data2[5]['T_SIZE'] = 8;$data2[6]['T_SIZE'] = 8;
						$data2[0]['LN_SIZE'] = $linespace;$data2[1]['LN_SIZE'] = $linespace;$data2[2]['LN_SIZE'] = $linespace;$data2[3]['LN_SIZE'] = $linespace;$data2[4]['LN_SIZE'] = $linespace;$data2[5]['LN_SIZE'] = $linespace;$data2[6]['LN_SIZE'] = $linespace;
	...
	case 4:
		...
		if(!empty($ar_data)) {
			...
			foreach($ar_data as $k => $v) {
				...
				$data1[3]['TEXT'] = $v1['wp_wr_nama_milik'];
				$data1[4]['TEXT'] = $v1['npwprd'];
				$data1[5]['TEXT'] = $v1['wp_wr_tgl_kartu'];
				if(!empty($wpwr_kdusaha)) $data1[6]['TEXT'] = "";
				else $data1[6]['TEXT'] = strtoupper($kodus);
				
						
				$data1[0]['T_ALIGN'] = 'R';$data1[3]['T_ALIGN'] = 'L';$data1[4]['T_ALIGN'] = 'C';$data1[5]['T_ALIGN'] = 'C';$data1[6]['T_ALIGN'] = 'C';
				$data1[0]['BRD_TYPE'] = 'LT';$data1[1]['BRD_TYPE'] = 'LT';$data1[2]['BRD_TYPE'] = 'LT';$data1[3]['BRD_TYPE'] = 'LT';$data1[4]['BRD_TYPE'] = 'LT';$data1[5]['BRD_TYPE'] = 'LT';$data1[6]['BRD_TYPE'] = 'LRT';
				$data1[0]['T_SIZE'] = 8;$data1[1]['T_SIZE'] = 8;$data1[2]['T_SIZE'] = 8;$data1[3]['T_SIZE'] = 8;$data1[4]['T_SIZE'] = 8;$data1[5]['T_SIZE'] = 8;$data1[6]['T_SIZE'] = 8;
				$data2[0]['LN_SIZE'] = $linespace;$data2[1]['LN_SIZE'] = $linespace;$data2[2]['LN_SIZE'] = $linespace;$data2[3]['LN_SIZE'] = $linespace;$data2[4]['LN_SIZE'] = $linespace;$data2[5]['LN_SIZE'] = $linespace;$data2[6]['LN_SIZE'] = $linespace;
