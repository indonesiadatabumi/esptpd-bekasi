...
class Dokumentasi_pengolahan extends Master_Controller
{
	...
	function daftar_induk_wpwr()
	{
		...
		$data['title'] = 'Pendaftaran | Dokumentasi';
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pendaftaran';
		$data['content_desc'] = 'Cetak Daftar Induk WP';
		$page = 'pendaftaran/form_daftar_induk_wpwr';
	...
	/**
	 * cetak daftar induk baru controller
	 */
	function daftar_induk_wpwr_baru()
	{
		$data['pejabat_daerah'] = $this->get_pejabat_daerah();
		$data['kecamatan'] = $this->get_kecamatan();
		$data['kelurahan'] = array('' => '--');
		...
		$data['content'] = 'Pendaftaran';
		$data['content_desc'] = 'Cetak Daftar Induk WP';
		$page = 'pendaftaran/form_daftar_induk_wpwr_baru';
		echo modules::run('layouts/loadview', $data, $page);

		// $this->load->view('form_daftar_induk_wpwr_baru', $data);
	}
	 ...
	 * export_daftar_induk
	 */
	function export_daftar_induk_baru()
	{
		$mengetahui = $this->input->get('mengetahui');
		$pemeriksa = $this->input->get('pemeriksa');
		...
		//load view pdf
		$this->load->view('pdf_daftar_induk_wpwr_baru', $data);
	}
	...
	function export_daftar_induk_xls()
	{
		...
		$worksheet1->write_string(0, 0, "Daftar Wajib Pajak");
		...
		$worksheet1->write_string(4, 4, "Pemilik");
		$worksheet1->write_string(4, 5, "Tgl. Daftar");

		//Prepare data
		$andwhere = "";
		if (!empty($_GET['wp_wr_golongan'])) {
			if ($_GET['wp_wr_golongan'] == "1") {
				$andwhere .= " AND b.wp_wr_gol='1'";
			} elseif ($_GET['wp_wr_golongan'] == "2") {
				$andwhere .= " AND b.wp_wr_gol='2'";
			}
		}
					...
					foreach ($ar_data as $k => $v) {
						...
						if (!empty($ardat)) {
							...
							foreach ($ardat as $k1 => $v1) {
								...
								$worksheet1->write_string($no_baris, 4, $v1['wp_wr_nama_milik']);
								$worksheet1->write_string($no_baris, 5, format_tgl($v1['wp_wr_tgl_terima_form']));
	 ...
	 *daftar wp baru
	 */
	function export_daftar_induk_baru_xls()
	{
		error_reporting(E_ERROR | E_WARNING | E_PARSE);
		...
		$worksheet1->write_string(0, 0, "Daftar Wajib Pajak baru");
		$worksheet1->write_string(1, 0, $fDate . " - " . $tDate);
	...
	function daftar_wpwr_tutup()
	{
		...
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pendaftaran';
		$data['content_desc'] = 'Cetak Daftar WP Tutup';
		$page = 'pendaftaran/form_daftar_wpwr_tutup';
		echo modules::run('layouts/loadview', $data, $page);
	...
	function daftar_perkembangan_wpwr()
	{
		...
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pendaftaran';
		$data['content_desc'] = 'Cetak Daftar Perkembangan WPWR';
		$page = 'pendaftaran/form_daftar_perkembangan_wpwr';
		echo modules::run('layouts/loadview', $data, $page);
