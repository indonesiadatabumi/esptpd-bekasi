...
$pdf->MultiCell(190, 4, $pdf->SetFont('Arial', '', 7) .
	...
	$pdf->SetFont('Arial', '', 10) . $pdf->Cell(140, 4, "TANGGAL : " . format_tgl($_GET['tgl_entry1'], false, true) . " s/d " . format_tgl($_GET['tgl_entry2'], false, true), '', '', 'C') .
	$pdf->Cell(48, 3, "", '', '', 'C'), '', '', '', 0);

$dt_kecamatan = ($kecamatan != null) ? "KECAMATAN : " . $kecamatan : "";
$pdf->MultiCell(190, 4, $pdf->SetFont('Arial', '', 7) .
	$pdf->Cell(12, 4, "", '', '', 'C') .
	$pdf->Cell(50, 2, "Telp. " . $pemda->dapemda_no_telp . ", Fax. " . $pemda->dapemda_no_fax, '', '', 'L') .
	$pdf->SetFont('Arial', '', 10) . $pdf->Cell(140, 4, $dt_kecamatan, '', '', 'C') .
	$pdf->Cell(48, 3, "", '', '', 'C'), '', '', '', 0);
	...
	foreach ($rows as $key => $val) {
		if ($_GET['spt_jenis_pajakretribusi'] == "3" || $_GET['spt_jenis_pajakretribusi'] == "7") {
			...
			if ($row_span > 0) {
				...
				$tbl2[6]['TEXT'] = getNamaBulan($arr_periode_jual[1]) . " " . $arr_periode_jual[0];

				$tbl2[0]['ROWSPAN'] = $row_span;
				$tbl2[1]['ROWSPAN'] = $row_span;
				$tbl2[2]['ROWSPAN'] = $row_span;
				$tbl2[3]['ROWSPAN'] = $row_span;
				$tbl2[4]['ROWSPAN'] = $row_span;
				$tbl2[5]['ROWSPAN'] = $row_span;
				$tbl2[6]['ROWSPAN'] = $row_span;
