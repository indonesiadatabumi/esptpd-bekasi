<div id="toolbar-box">
	...
	<div class="m">
		...
		<div class="header icon-48-electric">
			Isian SPTPD PPJ PLN: <small><small id='title_head'>[ Edit ]</small></small>
		</div>
		<div class="clr"></div>
	</div>
	<div class="b">
...
<div id="element-box">
	...
	<div class="m">
		...
		$attributes = array('id' => 'frm_edit_sptpd_pln');
		$hidden = array(
			'mode' => 'add',
			'spt_jenis_pajakretribusi' => $this->config->item('jenis_pajak_pln'),
			'kodus_id' => $this->config->item('kodus_pln'),
			'korek' => $this->config->item('korek_pln'),
			'spt_id' => $row->spt_id,
			'spt_dt_id' => $row->spt_dt_id
		);
		echo form_open('frm_edit_sptpd_pln', $attributes, $hidden);
		?>
					...
					<td valign="top">
						<table class="admintable" border=0 cellspacing="1">
							...
							<tr>
								...
								<td>
									<input type="text" name="spt_pajak" id="spt_pajak" value="<?= format_currency($row->spt_pajak); ?>" <?= $js; ?> class="inputbox" size="15" style="font-weight:bold;font-size:25px;color:#18F518;background-color:black;text-align:right;" onfocus="this.value=unformatCurrency(this.value);" onkeyup="getPengenaan('spt_pajak','spt_nilai','korek_persen_tarif');" onblur="getPengenaan('spt_pajak','spt_nilai','korek_persen_tarif');this.value=formatCurrency(this.value);" />
								</td>
							</tr>
							<tr>
								...
								<td>
									<?php
									$dasar_pengenaan = 0;
									if ($row->spt_dt_jumlah == 0)
										$dasar_pengenaan = (100 / $row->spt_dt_persen_tarif) * $row->spt_pajak;
									else
										$dasar_pengenaan = $row->spt_dt_jumlah;
									?>
									<input type="text" name="spt_nilai" id="spt_nilai" value="<?= format_currency($dasar_pengenaan); ?>" class="inputbox" size="20" readonly="readonly" style="text-align:right;" />
									...
									echo form_dropdown('korek_persen_tarif', $tarif, $row->spt_dt_persen_tarif, $attributes);
									?> %
								</td>
							</tr>
						</table>
...
<script type="text/javascript">
	GLOBAL_PAJAK_PLN_VARS["get_rekening"] = "rekening/popup_rekening";
	GLOBAL_PAJAK_PLN_VARS["update_sptpd"] = "<?= base_url(); ?>pendataan/pajak_pln/update";
</script>
<script type="text/javascript" src="<?= base_url(); ?>modules/pendataan/scripts/edit_pajak_pln.js"></script>