<div id="toolbar-box">
	...
	<div class="m">
		...
		<div class="header icon-48-user">
			Isian SKPD Pajak Air Tanah: <small><small id='title_head'>[ Edit ]</small></small>
		</div>
		<div class="clr"></div>
	</div>
	<div class="b">
...
<div id="element-box">
	...
	<div class="m">
		...
		$attributes = array('id' => 'frm_edit_sptpd_air');
		$hidden = array(
			'mode' => 'edit',
		...
		echo form_open('frm_edit_sptpd_air', $attributes, $hidden);
		?>
							...
							<table class="admintable" border=0 cellspacing="1">
												...
												<tbody id="tbody_detail">
													...
													?>
															<tr class="row0" id="row_detail<?= $counter ?>">
																...
																<td>
																	<input type="text" name="spt_dt_jumlah[]" id="spt_dt_jumlah<?= $counter ?>" class="inputbox" size="8" value="<?= $detail->spt_dt_jumlah; ?>" onchange="getPajak('spt_dt_pajak<?= $counter ?>','spt_dt_persen_tarif<?= $counter ?>','spt_dt_dasar_pengenaan<?= $counter; ?>','spt_dt_dasar_pengenaan<?= $counter; ?>','spt_dt_jumlah<?= $counter ?>');calc1();" style="text-align:right;" autocomplete="off" />
																</td>
																<td>
																	<input type="text" name="spt_dt_dasar_pengenaan[]" id="spt_dt_dasar_pengenaan<?= $counter ?>" value="<?= format_currency($detail->spt_dt_tarif_dasar) ?>" class="inputbox" size="16" onchange="getPajak('spt_dt_pajak<?= $counter ?>','spt_dt_persen_tarif<?= $counter ?>','spt_dt_dasar_pengenaan<?= $counter; ?>','spt_dt_dasar_pengenaan<?= $counter; ?>','spt_dt_jumlah<?= $counter ?>');calc1();" onblur="this.value=formatCurrency(this.value);" onfocus="this.value=unformatCurrency(this.value);" style="text-align:right;" autocomplete="off" />
																</td>
																<td align="right">
																	<input type="text" name="spt_dt_persen_tarif[]" id="spt_dt_persen_tarif<?= $counter ?>" size="2" class="inputbox" value="<?= $detail->spt_dt_persen_tarif ?>" readonly="true" style="text-align:right;" />
																</td>
																<td>
								...
								<tr>
									<td align="center">
										...
										&nbsp;&nbsp;&nbsp; TOTAL &nbsp;&nbsp;&nbsp;
										<input type="text" name="spt_pajak" id="spt_pajak" class="inputbox" size="20" value="<?= format_currency($row->spt_pajak); ?>" readonly="true" style="font-weight:bold;font-size:25px;color:#18F518;background-color:black;text-align:right;" />
									</td>
								</tr>
							</table>
						</fieldset>
...
<script type="text/javascript">
	GLOBAL_PAJAK_AIR_VARS["get_rekening"] = "<?= base_url(); ?>rekening/get_arr_list";
	GLOBAL_PAJAK_AIR_VARS["update_sptpd"] = "<?= base_url(); ?>pendataan/pajak_air_bawah_tanah/update";
</script>
<script type="text/javascript" src="<?= base_url(); ?>modules/pendataan/scripts/edit_pajak_air_bawah_tanah.js"></script>