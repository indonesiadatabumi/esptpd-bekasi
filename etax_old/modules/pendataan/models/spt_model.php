...
class Spt_model extends CI_Model {
	...
	function get_nomor_spt($periode, $spt_jenis_pajak) {
		$sql = "SELECT COALESCE(max(spt_nomor)+1,1) as max_no_spt FROM spt 
						WHERE spt_periode='".$periode."' 
						AND spt_jenis_pajakretribusi='".$spt_jenis_pajak."'";
	...
	function delete_spt($spt_id) {
		...
		$spt_row = $this->db->query("SELECT spt_idwp_reklame FROM spt WHERE spt_id='".$spt_id."'")->row_array();

		$this->db->delete('wp_wr_reklame',array('wp_rek_id'=>$spt_row['spt_idwp_reklame']));
		...
		if($this->db->affected_rows() > 0){
			...
			if ($this->db->affected_rows() > 0) {
				...
				$spt_detail_query = $this->db->query("SELECT spt_dt_id FROM spt_detail WHERE spt_dt_id_spt='".$spt_id."'");
				
				foreach($spt_detail_query->result_array() as $row){
	...
	function delete_spt_all($spt_id) {
		$this->db->delete('spt', array('spt_id' => $spt_id)); 
		if ($this->db->affected_rows() > 0) {
	...
	function insert_history($spt_id)
	{
		$spt_row = $this->db->query("SELECT * FROM spt WHERE spt_id='".$spt_id."'")->row_array();
		$arr_spt = array(
			'spt_id' => $spt_row['spt_id'],
			'spt_periode' => $spt_row['spt_periode'],
			'spt_nomor' => $spt_row['spt_nomor'],
			'spt_kode_rek' => $spt_row['spt_kode_rek'],
			'spt_tgl_terima' => $spt_row['spt_tgl_terima'],
			'spt_tgl_bts_kembali' => $spt_row['spt_tgl_bts_kembali'],
			'spt_nama_penerima' => $spt_row['spt_nama_penerima'],
			'spt_alamat_penerima' => $spt_row['spt_alamat_penerima'],
			'spt_periode_jual1' => $spt_row['spt_periode_jual1'],
			'spt_periode_jual2' => $spt_row['spt_periode_jual2'],
			'spt_status' => $spt_row['spt_status'],
			'spt_nilai' => $spt_row['spt_nilai'],
			'spt_pajak' => $spt_row['spt_pajak'],
			'spt_operator' => $spt_row['spt_operator'],
			'spt_jenis_pajakretribusi' => $spt_row['spt_jenis_pajakretribusi'],
			'spt_idwpwr' => $spt_row['spt_idwpwr'],
			'spt_jenis_pemungutan' => $spt_row['spt_jenis_pemungutan'],
			'spt_tgl_proses' => $spt_row['spt_tgl_proses'],
			'spt_tgl_entry' => $spt_row['spt_tgl_entry'],
			'spt_tarif_persen' => $spt_row['spt_tarif_persen'],
			'spt_tarif_dasar' => $spt_row['spt_tarif_dasar'],
			'spt_no_register' => $spt_row['spt_no_register'],
			'spt_idwpwr_detail' => $spt_row['spt_idwpwr_detail'],
			'spt_kode' => $spt_row['spt_kode'],
			'spt_idwp_reklame' => $spt_row['spt_idwp_reklame'],
			'spt_kode_billing' => $spt_row['spt_kode_billing'],
			'status_bayar' => $spt_row['status_bayar']
		);
        $this->db->insert('history_del_spt', $arr_spt);
	}
}