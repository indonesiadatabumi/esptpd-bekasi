		...
		if ($this->input->get('daftar_spt') == '2' || $this->input->get('daftar_spt') == '0') {
			...
			for ($i=$masa_pajak; $i <= $masa_pajak2 ; $i++) {
				...
				if ($spt_jenis_pajakretribusi == "3" || $spt_jenis_pajakretribusi == "7") {
					if (!empty($kecamatan)){
						$query[] = "SELECT a.wp_wr_nama, a.wp_wr_almt, a.npwprd, b.spt_id, b.spt_tgl_proses, b.spt_nomor, korek_persen_tarif, COALESCE(spt_periode_jual1, '$tahun-$i-01') as spt_periode_jual1
					...
					if (!empty($kecamatan)){
						$query[] = "SELECT a.wp_wr_nama, a.wp_wr_almt, a.npwprd, b.spt_id, b.spt_tgl_proses, b.spt_nomor, korek_persen_tarif, COALESCE(spt_periode_jual1, '$tahun-$i-01') as spt_periode_jual1, b.spt_dt_persen_tarif, b.spt_dt_jumlah, b.spt_dt_pajak
						FROM v_wp_wr a LEFT JOIN v_sptpd_list b 
						ON a.wp_wr_id = b.wp_wr_id AND spt_jenis_pajakretribusi='".$spt_jenis_pajakretribusi."' 
						AND spt_jenis_pemungutan='".$sistem_pemungutan."' 
						AND EXTRACT(MONTH FROM spt_periode_jual1)='$i' 
			...
			if ($spt_jenis_pajakretribusi == "3" || $spt_jenis_pajakretribusi == "7") {
				$sql_cari = "SELECT DISTINCT wp_wr_nama, wp_wr_almt, npwprd, spt_id, spt_tgl_proses, spt_nomor, spt_periode_jual1
					FROM v_sptpd_list 
