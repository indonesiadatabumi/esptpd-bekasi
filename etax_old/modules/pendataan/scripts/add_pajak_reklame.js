var add_detail_row = function(){
    ...
    new_row += "<tr><td class='key'>Area</td><td><input type='radio' name='area"+new_order+"' id='area"+new_order+"_1' value='1' onchange=\"execute_area_function("+new_order+")\" checked='checked' />&nbsp;Outdoor&nbsp;&nbsp;"+
			   "<input type='radio' name='area"+new_order+"' id='area"+new_order+"_2' onchange=\"execute_area_function("+new_order+")\" value='2' />&nbsp;Indoor</td></tr><tr>";
	new_row += "<tr><td class='key'>Tembakau dan Minuman Keras</td><td><input type='checkbox' name='tmk"+new_order+"' id='tmk"+new_order+"_1' value='1' onchange=\"execute_tmk_function("+new_order+")\" />&nbsp;Rokok&nbsp;"+
			   "<input type='checkbox' name='tmk"+new_order+"' id='tmk"+new_order+"_2' value='2' onchange=\"execute_tmk_function("+new_order+")\"  />&nbsp;Minuman keras&nbsp;</td></tr>";
	new_row += "<tr><td class='key2'>Penambahan ketinggian</td><td><input type='checkbox' name='tinggi"+new_order+"' id='tinggi"+new_order+"_1' value='1' onchange=\"execute_tinggi_function("+new_order+")\" />&nbsp;Tinggi&nbsp;</td></tr>";
	new_row += "<tr><td class='key'>Naskah / Judul</td><td>"+
...
var insertOptionRekening = function(idSelect) {	
	$.get(
		GLOBAL_PAJAK_REKLAME_VARS["get_rekening"], 
		{ koderek : $("input[name=korek]").val() }, 
		function(data){
			var option = "";
...
var calcNSR = function(row_number) {
	...
	var nsr_ = ($("#txt_luas"+row_number).val() * 1) * ($("#txt_jumlah"+row_number).val() * 1) * ($("#txt_jangka_waktu"+row_number).val() * 1) * ($('#txt_nilai_kelas_jalan'+row_number).val().toString().replace(/\$|\,00/g,'').replace(/\$|\./g,'') * 1);
	
	}
	else if (no_rekening == "4.1.1.04.05") {  
	
	var nsr_ = ($("#txt_luas"+row_number).val() * 1) * ($("#txt_jangka_waktu"+row_number).val() * 1) * ($('#txt_nilai_tarif'+row_number).val().toString().replace(/\$|\,00/g,'').replace(/\$|\./g,'') * 1) ;
		
	}
	
	var tng = 0;

	if ($("input[name=tinggi"+row_number+"]:checked").val() == "1"){
	...
	var tm= 0 ;
	if ($("input[name=tmk"+row_number+"]:checked").val() == "1"   ){
	tm = jumlah_pajak * 0.5;

	jumlah_pajak=jumlah_pajak+tm;

	$('#spt_sub_pajak'+row_number).val(formatCurrency(Math.ceil(parseInt(jumlah_pajak) / 100) * 100));
	}else if ($("input[name=tmk"+row_number+"]:checked").val() ==  "2"  ){
		tm = jumlah_pajak * 0.5;
...
var calcPajak = function(row_number) {
	...
	if (no_rekening == "4.1.1.04.06") {
		durasi = parseInt($("#txt_durasi"+row_number).val()*1);
		nilai = durasi * jumlah * ($('#txt_nilai_tarif'+row_number).val().toString().replace(/\$|\,00/g,'').replace(/\$|\./g,'') * 1);
	...
	if ($("input[name=tmk"+row_number+"]").is(':checked'))
	nilai = (nilai * 0.5) + nilai;
...
var completePage = function() {
	...
	insertOptionRekening('spt_dt_korek1');
	...
	// $("#spt_dt_korek1").change(function(){

	// 	var spt_dt_korek = $("#spt_dt_korek1 :selected");
	// 	var no_rekening = spt_dt_korek.text().substring(1, 12); 
	// 	$("#detail_nsr1").html(row_jenis_reklame(no_rekening));
		
	// 	arr_spt_korek = spt_dt_korek.val().split(",");
		
	// 	if (no_rekening == "4.1.1.04.01" || no_rekening == "4.1.1.04.02") {
	// 		$("#row_persen_tarif1").show();
	// 		$("#spt_dt_persen_tarif1").val(arr_spt_korek[1]);
	// 		insertKelasJalan();
	// 	} else {
	// 		$("#row_persen_tarif1").hide();
	// 		$("#spt_dt_persen_tarif1").val(arr_spt_korek[1]);
	// 		$("#txt_nilai_tarif1").val(formatCurrency(arr_spt_korek[2]));
	// 	}
		

	// 	$("#spt_pajak").val(formatCurrency("0"));
	// 	$("input[name=txt_korek1]").val(no_rekening);
	// });
	
	// $("input[name=area1]").change(function(){
	// 	no_rekening = $("#spt_dt_korek1 :selected").text().substring(1, 12); 
	// 	if (no_rekening == "4.1.1.04.01" || no_rekening == "4.1.1.04.02") {
	// 		calcNSR();
	// 	} else {
	// 		calcPajak();
	...
	// $("#chb_perda_lama1").click(function() {
	// 	if ($("#chb_perda_lama1").is(":checked")) {
	// 		$("#spt_pajak1").removeAttr("readonly");
	// 	} else {
	// 		$("#spt_pajak1").attr("readonly", "readonly");
	// 		$("#spt_pajak1").val(formatCurrency("0"));
	// 	}
	// });
};
...
var execute_area_function = function(row_number){
	...
	if (no_rekening == "4.1.1.04.01" || no_rekening == "4.1.1.04.02"  || no_rekening == "4.1.1.04.05"  || no_rekening == "4.1.1.04.12") {
		calcNSR(row_number);
	} else {
...
var execute_tmk_function = function(row_number){
	no_rekening = $("#spt_dt_korek"+row_number+" :selected").text().substring(1, 12); 
	if (no_rekening == "4.1.1.04.01" || no_rekening == "4.1.1.04.02" || no_rekening == "4.1.1.04.05"  || no_rekening == "4.1.1.04.12") {
		calcNSR(row_number);
	} else {
...
var execute_tinggi_function = function(row_number){
	no_rekening = $("#spt_dt_korek"+row_number+" :selected").text().substring(1, 12); 
	if (no_rekening == "4.1.1.04.01" || no_rekening == "4.1.1.04.02" || no_rekening == "4.1.1.04.05"  || no_rekening == "4.1.1.04.12") {
		calcNSR(row_number);
	} else {
	...
	if (no_rekening == "4.1.1.04.01" || no_rekening == "4.1.1.04.02" || no_rekening == "4.1.1.04.12") {
		...
		rowField = 
			'<table>'+
				...
				'<tr>'+
					'<td class="key">Jumlah Pemasangan</td>' +
					'<td><input type="text" name="txt_jumlah'+row_number+'" id="txt_jumlah'+row_number+'" class="inputbox" size="5" onchange="calcPajak('+row_number+');" onblur="calcPajak('+row_number+');"> unit</td>' +
				'</tr>' +
				
                		'<tr>'+
					'<td class="key">Lama Pemasangan</td>' +
					'<td><input type="text" name="txt_durasi'+row_number+'" id="txt_durasi'+row_number+'" class="inputbox" size="6" onchange="calcPajak('+row_number+');" onblur="calcPajak('+row_number+');"> bulan</td>' +
				'</tr>' +				
				'<tr>'+
		...
		rowField = 
			'<table>'+
				...
				'</tr>' +
			'</table>';
	} else if (no_rekening == "4.1.1.04.03") {
		rowField = 
			'<table>'+
				'<tr>'+
					'<td class="key">Ukuran</td>' +
					'<td><input type="text" name="txt_luas'+row_number+'" id="txt_luas'+row_number+'" class="inputbox" size="8" onchange="calcPajak('+row_number+');"> Cm<sup>2</td>' +
				'</tr>' +
				...
				'</tr>' +
			'</table>';
	} else if (no_rekening == "4.1.1.04.04") {
		rowField = 
			'<table>'+
				'<tr>'+
					'<td class="key">Luas</td>' +
...
var createEventToolbar = function() {
	var submitForm = function() {
		...
		var save_options = { 
			url : GLOBAL_PAJAK_REKLAME_VARS["insert"],
			type : "POST",
			dataType: 'json',
		...
		$("#frm_add_reklame").ajaxSubmit(save_options);
	};
	...
	$("#btn_view").click(function() {
		//load content panel
		load_content(GLOBAL_PAJAK_REKLAME_VARS["view"]);
	});
	
	//button import
	$("#btn_import").click(function() {
		//load content panel
		load_content(GLOBAL_PAJAK_REKLAME_VARS["view_import"]);
	});
};
