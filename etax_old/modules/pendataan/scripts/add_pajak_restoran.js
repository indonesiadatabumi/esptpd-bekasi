...
var completePage = function() {
	...
	$('#wp_wr_kode_lurah').autotab({ target: 'jenis_pajak_restoran', format: 'numeric', previous: 'wp_wr_kd_camat' });
	$('#jenis_pajak_restoran').autotab({ target: 'fDate', format: 'text', previous: 'wp_wr_kode_lurah' });
	
	$('#korek_rincian').autotab({ target: 'korek_sub1', format: 'numeric' });
	$('#korek_sub1').autotab({ target: 'spt_nilai', format: 'numeric', previous: 'korek_rincian' });
	...
	$("#trigger_rek").click(function() {
		showDialog(GLOBAL_PAJAK_RESTORAN_VARS["get_rekening"]+"?koderek=" + $("#korek").val(), 'Kode Rekening', 800, 500);
	});
...
/**
 * get rekening restoran
 * @returns
 */
var getRekeningRestoran = function() {
	$.ajax({
		type: "POST",
...
var createEventToolbar = function() {
	var submitForm = function() {
		var showInsertResponse = function (response, status) {
            if(response.status == true) {
            	...
            	$("#frm_add_sptpd_restoran").resetForm();
            	getNextNomor();
            	getRekeningRestoran();
            	completePage();
            } else {
		...
		var save_options = { 
			url : GLOBAL_PAJAK_RESTORAN_VARS["insert_sptpd"],
			type : "POST",
			dataType: 'json',
		...
		$("#frm_add_sptpd_restoran").ajaxSubmit(save_options);
	};
	...
	$("#btn_view").click(function() {
		//load content panel
		load_content(GLOBAL_PAJAK_RESTORAN_VARS["view_sptpd"]);
	});
};
