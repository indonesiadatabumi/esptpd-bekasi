...
var appendRowField = function(id) {
	var rowField = 
		'<tr class="row0" id="row_detail'+ id +'">'+
			...
			'<input type="hidden" name="lhp_dt_id_spt[]" id="lhp_dt_id_spt'+ id +'">'+
			...
			'<input type="text" name="lhp_dt_dsr_pengenaan[]" id="lhp_dt_dsr_pengenaan'+ id +'" class="mandatory" onblur="calcTerhutang('+ id +');this.value=formatCurrency(this.value);" onfocus="this.value=unformatCurrency(this.value);" class="numerics" size="17" value=""  style="text-align:right;"/>'+
			'</td>'+
			'<td align="right" valign="top">'+
...
var completePage = function() {
	...
	$('#btn_npwpd').click(function() {
		showDialog("wajib_pajak/popup_wp_by_jenis_pajak/?kodus=0" + $("#lhp_jenis_pajakretribusi").val(), '', 1000, 500);
	});	
	...
	var insert = function() {
		var showInsertResponse = function (response, status) {
            if(response.status == true) {
            	...
            	$("#frm_add_lhp").resetForm();
            } else {
            	showWarning(response.msg);
            }
            $('#btn_save').one("click", insert);
		};
		
		var save_options = { 
			url : GLOBAL_LHP_VARS["insert"],
			type : "POST",
			dataType: 'json',
			beforeSubmit: function(arr, $form, options) { 
				validate = jqform_validate(arr, $form, options);
				if(!validate)	$('#btn_save').one("click", insert);
				return validate;
			},
			success: showInsertResponse
		};
		
		$("#frm_add_lhp").ajaxSubmit(save_options);
	};
