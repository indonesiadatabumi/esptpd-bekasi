 ...
 * @version 20121016
 */
class Pajak_genset extends Master_Controller {
	/**
	 * contructor
	 */
	function __construct() {
		parent::__construct();
		$this->load->model('pajak_genset_model');
	...
	function add() {
		...
		$this->load->view('add_pajak_genset', $data);
	}
		...
		if (!empty($wp_id)) {
			...
			if (!$check_masa_pajak) {
				...
				if ($spt_id != 0) {
					...
					//insert history log
					$this->common_model->history_log("pendataan", "i", "Insert SPT Pajak Genset : ".
	 ...
	 */
	function view() {
		$this->load->view('view_pajak_genset');	
	}
	 ...
	 */
	function get_list() {
		$this->pajak_genset_model->get_list();
	}
	...
	function edit() {
		...
		if ($sptpd != false) {
			//get spt_detail
			$spt_detail = $this->pajak_genset_model->get_spt_detail();
			$data['row'] = $sptpd;
			$data['row_detail'] = $spt_detail;
			$this->load->view('edit_pajak_genset', $data);	
		}
	}
		...
		if (!empty($wp_id) && !empty($spt_id)) {
			...
			if (!$check_masa_pajak) {
				...
				if ($return) {
					...
					//insert history log
					$this->common_model->history_log("pendataan", "U", "Update SPT Pajak Genset : ".
	...
	function delete() {
		...
		for ($i = 0; $i < count($arr_id) - 1; $i++) {
			$this->spt_model->insert_history($arr_id[$i]);
			if($this->spt_model->delete_spt_all($arr_id[$i]) == true) {
				$counter++;
				//insert history log
				$this->common_model->history_log("pendataan", "D", "Delete SPT Pajak Genset : ".$arr_id[$i]);
			}
		}
