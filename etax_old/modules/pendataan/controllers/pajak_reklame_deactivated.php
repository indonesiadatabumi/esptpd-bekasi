...
class Pajak_reklame extends Master_Controller {
		...
		if ($wp_reklame_id != false) {
			...
			if (!$check_masa_pajak) {			
				...
				if ($spt_id != 0) {
					...
					$spt_dt_id = $this->pajak_reklame_model->insert_spt_detail($spt_id);
					
					if ($spt_dt_id != false)
						//insert into spt_reklame
						$this->pajak_reklame_model->insert_spt_reklame($spt_dt_id);			
	...
	function edit() {
		...
		if ($sptpd != false) {
			//get spt_detail
			$spt_detail = $this->pajak_reklame_model->get_spt_detail();
			$data['row'] = $sptpd;
			$data['row_detail'] = $spt_detail->row();
	 ...
	 */	
		...
		if (!empty($wp_id) && !empty($spt_id)) {
			...
			if ($return) {
				$spt_dt_id = $this->pajak_reklame_model->update_spt_detail($this->input->post('spt_dt_id'));
				
				if ($spt_dt_id != false)
					//update into spt_reklame
					$this->pajak_reklame_model->update_spt_reklame($this->input->post('sptrek_id'), $this->input->post('spt_dt_id'));			
	...
	function delete() {
		...
		for ($i = 0; $i < count($arr_id) - 1; $i++) {
			...
			if($this->spt_model->delete_spt($arr_id[$i]) == true) {
				$this->pajak_reklame_model->delete_wp_reklame($arr_id[$i]);
				$counter++;
