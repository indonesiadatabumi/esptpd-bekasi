 ...
 */
class Lhp_telat_lapor_model extends CI_Model
{
	/**
	 * get list LHP
	 */
	function get_list_lhp()
	{
		...
		$tables = " v_lhp_telat_lapor LEFT JOIN keterangan_spt ket ON v_lhp_telat_lapor.lhp_jenis_ketetapan=ket.ketspt_id ";
		$relations = "";
		$where = "";
	...
	function get_lhp_data()
	{
		...
		$sql = "SELECT *
					FROM lhp_telat_lapor lhp
					JOIN lhp_telat_lapor_detail lhpd ON lhp.lhp_id=lhpd.lhp_dt_id_header
	...
	{
		$result = array();
		$this->db->select_max('lhp_id');
		$query = $this->db->get('lhp_telat_lapor');
		$return = $query->row();

		$lhp_id = $return->lhp_id;
		$lhp_id_final = (int)$lhp_id + 1;
		//save header
		$arr_lhp = array(
			'lhp_id' => $lhp_id_final,
		...
		$this->db->insert('lhp_telat_lapor', $arr_lhp);
		...
		if ($this->db->affected_rows() > 0) {
			...
			if (!empty($_POST['lhp_dt_periode1'])) {
				foreach ($_POST['lhp_dt_periode1'] as $k => $v) {
					//insert lhp detail
					$detail = array();
					$this->db->select_max('lhp_dt_id');
					$query = $this->db->get('lhp_telat_lapor_detail');
					$return = $query->row();

					$lhp_dt_id = $return->lhp_dt_id;
					$lhp_dt_id_final = (int)$lhp_dt_id + 1;
					$detail["lhp_dt_id"] = $lhp_dt_id_final;
					$detail["lhp_dt_pajak"] = unformat_currency($_POST['pajak_terhutang'][$k]);
					$detail["lhp_dt_id_header"] = $lhp_id_final;
					...
					$this->db->insert('lhp_telat_lapor_detail', $detail);
				}
			}
	...
	function edit()
	{
		...
		$this->db->update('lhp_telat_lapor', $arr_lhp);
			...
			if (!empty($_POST['lhp_dt_periode1'])) {
				foreach ($_POST['lhp_dt_periode1'] as $k => $v) {
					...
					if (!empty($_POST['lhp_dt_id'][$k])) {
						...
						$this->db->update('lhp_telat_lapor_detail', $detail);
					} else {
						//insert lhp detail
						$this->db->select_max('lhp_dt_id');
						$query = $this->db->get('lhp_telat_lapor_detail');
						...
						$this->db->insert('lhp_telat_lapor_detail', $arr_detail);
					}
				...
				$delete = "DELETE FROM lhp_telat_lapor_detail WHERE lhp_dt_id NOT IN ($list_id) AND lhp_dt_id_header='$lhp_id'";
	...
	{
		$this->db->delete('lhp_telat_lapor', array('lhp_id' => $lhp_id));
		if ($this->db->affected_rows() > 0) {
			$this->db->delete('lhp_telat_lapor_detail', array('lhp_dt_id_header' => $lhp_id));
