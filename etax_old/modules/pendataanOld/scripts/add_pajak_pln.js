...
var completePage = function() {	
	...
	$("#trigger_rek").click(function() {
		showDialog(GLOBAL_PAJAK_PLN_VARS["get_rekening"]+"?koderek=" + $("#korek").val(), 'Kode Rekening', 800, 500);
	});
...
var createEventToolbar = function() {
	var submitForm = function() {
		...
		var save_options = { 
			url : GLOBAL_PAJAK_PLN_VARS["insert_sptpd"],
			type : "POST",
			dataType: 'json',
		...
		$("#frm_add_sptpd_pln").ajaxSubmit(save_options);
	};
	...
	$("#btn_view").click(function() {
		//load content panel
		load_content(GLOBAL_PAJAK_PLN_VARS["view_sptpd"]);
	});
};

var getRekeningPln = function() {
	$.post(GLOBAL_PAJAK_PLN_VARS["rekening_pln"], {
