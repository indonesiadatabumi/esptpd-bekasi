...
var createEventToolbar = function() {
	var submitForm = function() {
		var showInsertResponse = function (response, status) {
            if(response.status == true) {
            	...
            	$("#frm_add_sptpd_hiburan").resetForm();
            	getNextNomor();
            	completePage();
            } else {
            	showWarning(response.msg);
		...
		var save_options = { 
			url : GLOBAL_PAJAK_HIBURAN_VARS["insert_sptpd"],
			type : "POST",
			dataType: 'json',
		...
		$("#frm_add_sptpd_hiburan").ajaxSubmit(save_options);
	};
	...
	$("#btn_view").click(function() {
		//load content panel
		load_content(GLOBAL_PAJAK_HIBURAN_VARS["view_sptpd"]);
	});
};
