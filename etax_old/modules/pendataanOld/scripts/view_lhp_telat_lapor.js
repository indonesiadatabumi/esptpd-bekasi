 ...
 var createDataGrid = function () {
	$("#hasil_pemeriksaan_table").flexigrid({
	  ...
	  colModel: [
		...
		{
		  display:
			'<input type=checkbox name="toggle" value="" onclick="selectRows();" />',
		  name: "",
		  width: 30,
		  align: "center",
		  process: cellClick,
		...
		{
		  display: "Tgl. Pemeriksaan",
		  name: "lhp_tgl_periksa",
		  width: 100,
		  sortable: true,
		  align: "center",
		  process: cellClick,
	  ...
	  sortorder: "desc",
	  usepager: true,
	  title: "DAFTAR HASIL PEMERIKSAAN TELAT LAPOR",
	  useRp: true,
	  rp: 10,
  ...
  var deleteData = function () {
	var remove = function () {
	  ...
	  $.ajax({
		type: "POST",
		url: GLOBAL_MAIN_VARS["BASE_URL"] + "pendataan/lhp_telat_lapor/delete",
		data: "mode=delete&id=" + selectedItems,
		success: function (msg) {
		  $("#lhp_telat_lapor_table").flexReload();
