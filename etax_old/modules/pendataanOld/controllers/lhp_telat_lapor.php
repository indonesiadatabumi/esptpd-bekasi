 ...
 * @version 20121016
 */
class Lhp_telat_lapor extends Master_Controller
{
	/**
	 * constructor
	 */
	...
	{
		parent::__construct();
		$this->load->model('lhp_telat_lapor_model');
	...
	function index()
	{
		...
		$data['title'] = 'Pendataan | Hasil Pemeriksaan';
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pendataan';
		$data['content_desc'] = 'Input Laporan Hasil Pemeriksaan Telat Lapor';
		$page = 'pendataan/add_lhp_telat_lapor';
	...
	function get_list()
	{
		$this->lhp_telat_lapor_model->get_list_lhp();
	}
	...
	function view()
	{
		$this->load->view('view_lhp_telat_lapor');
	}
	...
	function save()
	{
		...
		*/
		...
		//if (!$check_lhp) {
		$this->lhp_telat_lapor_model->insert();
		//} else {
	...
	function edit()
	{
		...
		$data['jenis_pajak'] = $this->objek_pajak_model->get_ref_jenis_pajak_retribusi(false);
		$data['row'] = $this->lhp_telat_lapor_model->get_lhp_data();
		$data['row_detail'] = $this->lhp_telat_lapor_model->get_lhp_detail();
		$this->load->view('edit_lhp_telat_lapor', $data);
	}

	function update()
	{
		$this->lhp_telat_lapor_model->edit();
	}
	...
	function delete()
	{
		...
		for ($i = 0; $i < count($arr_id) - 1; $i++) {
			if ($this->lhp_telat_lapor_model->delete($arr_id[$i]) == true)
				$counter++;
		}
