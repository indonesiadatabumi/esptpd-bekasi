...
class Sptpd extends Master_Controller
{
	...
	function daftar()
	{
		...
		$data['title'] = 'Pendataan | Daftar sptpd';
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pendataan';
		$data['content_desc'] = '';
		$page = 'pendataan/form_daftar_sptpd';
		echo modules::run('layouts/loadview', $data, $page);
	...
	/**
	 * cetak daftar excel sptpd by tanggal entry
	 */
	function cetak_xls_daftar_sptpd_entry()
	{
		if ($this->input->get('kecamatan'))
			$data['kecamatan'] = $this->common_model->get_record_value('camat_nama', 'kecamatan', "camat_id='" . $this->input->get('kecamatan') . "'");
		...
		$this->load->view('xls_daftar_sptpd_entry', $data);
	}
