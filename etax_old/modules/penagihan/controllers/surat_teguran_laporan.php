 ...
 * @package Simpatda
 */
class Surat_teguran_laporan extends Master_Controller
{
	/**
	 * constructor
	 */
	function __construct()
	{
		...
		$this->load->model('surat_teguran_laporan_model');
	}
	...
	function index()
	{
		...
		$data['title'] = 'Penagihan | Surat Teguran Pelaporan';
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Penagihan';
		$data['content_desc'] = '';
		$page = 'penagihan/surat_teguran_laporan';
		echo modules::run('layouts/loadview', $data, $page);
		// $this->load->view('surat_teguran_laporan', $data);
	}


	function pdf_surat_teguran_laporan()
	{
		...
		$data['model'] = $this->surat_teguran_laporan_model;

		//insert history log
		$this->common_model->history_log("pembukuan", "P", "Cetak surat teguran : " . $_GET['spt_jenis_pajakretribusi'] . " | " . $_GET['bulan'] . "|" . $_GET['tahun'] . " | " . $_GET['wp_wr_kd_camat']);

		$this->load->view('pdf_surat_teguran_laporan', $data);
	}
	...
	function daftar()
	{
		...
		$data['kecamatan'] = $this->get_kecamatan();
		$data['pejabat_daerah'] = $this->get_pejabat_daerah();
		$this->load->view('daftar_surat_teguran_laporan', $data);
	}

	function pdf_daftar_surat_teguran_laporan()
	{
		$query = $this->common_model->get_query('*', 'data_pemerintah_daerah');
		$data['pemda'] = $query->row();
		...
		//insert history log
		$this->common_model->history_log("pembukuan", "P", "Cetak daftar surat teguran : " . $_GET['spt_jenis_pajakretribusi'] . " | " . $_GET['bulan'] . "|" . $_GET['tahun'] . " | " . $_GET['wp_wr_kd_camat']);

		$this->load->view('pdf_daftar_surat_teguran_laporan', $data);
	}
