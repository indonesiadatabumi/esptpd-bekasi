...
$pdf = new FPDF_TABLE('P','mm','Letter'); // Potrait,Letter,Lebar = 190
$pdf->Open();
$pdf->SetAutoPageBreak(true, 10); // Page Break dengan margin bawah 2 cm
$pdf->SetMargins(12, 10, 10);     // Margin Left, Top, Right 2 cm
...
if (!empty($arr_data)) {
	foreach ($arr_data as $k1 => $v1) {
		...
		$rekening = "";
		foreach($data_skpd as $row_key=>$row_val){
			if($rekening!=$row_val['rekening']){
				foreach($row_val as $key=>$val){
					$_data_skpd[$row_key][$key] = $val;
				}
			}
			$rekening = $row_val['rekening'];
		}
		//print_r($_data_skpd);
		...
		$pdf->SetStyle("h2","arial","B",15,"0,0,0");
		...
		$title = ucwords($_data_skpd[0]['ketspt_ket']);
		$s_title = strtoupper($_data_skpd[0]['ketspt_singkat']); 
		...
		$artgl = explode("-",$_data_skpd[0]['spt_periode_jual1']);
		$blnxa = $artgl[1];
		$tahun_pajak = $artgl[0];
		...
		$idn[0]['WIDTH'] = 5;$idn[1]['WIDTH'] = 21.5;$idn[2]['WIDTH'] = 5;$idn[3]['WIDTH'] = 158.5;
		...
		$tbd[1]['TEXT'] = "Nama";
		$tbd[2]['TEXT'] = ":";
		$tbd[3]['TEXT'] = "".strtoupper($_data_skpd[0]['wp_wr_nama']);
		$tbd[0]['BRD_TYPE'] = 'L';
		$tbd[3]['BRD_TYPE'] = 'R';
		$pdf->tbDrawData($tbd);
		
		$wp_wr_lurah = ($_data_skpd[0]['wp_wr_lurah'] == "") ? "" : "KEL. ".$_data_skpd[0]['wp_wr_lurah'];
		$wp_wr_camat = ($_data_skpd[0]['wp_wr_camat'] == "") ? "" : "KEC. ".$_data_skpd[0]['wp_wr_camat'];
		
		$tbd[0]['TEXT'] = "";
		$tbd[1]['TEXT'] = "Alamat";
		$tbd[2]['TEXT'] = ":";
		// $tbd[3]['TEXT'] = "".strtoupper(ereg_replace("\r|\n","",$_data_skpd[0][wp_wr_almt]))." ".strtoupper($wp_wr_lurah." ".$wp_wr_camat);
		$tbd[3]['TEXT'] = "".$data_skpd[0]['wp_wr_almt']." ".$wp_wr_lurah." ".$wp_wr_camat;
		$tbd[1]['V_ALIGN'] = 'T';
		$tbd[2]['V_ALIGN'] = 'T';
		$pdf->tbDrawData($tbd);

		if ($_data_skpd[0]['spt_jenis_pajakretribusi'] != '4') {
			$tbd[0]['TEXT'] = "";
			$tbd[1]['TEXT'] = "NPWPD";
			$tbd[2]['TEXT'] = ":";
			$tbd[3]['TEXT'] = "".$_data_skpd[0]['npwprd'];
			$pdf->tbDrawData($tbd);
		}
		
		$arr_tgl = explode("-", $_data_skpd[0]['netapajrek_tgl_jatuh_tempo']);
		$conv_tgl = $arr_tgl[2]."-".$arr_tgl[1]."-".$arr_tgl[0];
		
		$tbd[0]['TEXT'] = "";
		$tbd[1]['TEXT'] = "Jatuh Tempo : ". format_tgl($conv_tgl,false,true);
		$tbd[1]['COLSPAN'] = 4;
		$tbd[3]['BRD_TYPE'] = 'R';
		$pdf->tbDrawData($tbd);
		...
		$tbd[1]['COLSPAN'] = 2;
		$tbd[3]['TEXT'] = "". $_data_skpd[0]['spt_kode_billing']."\n\n";
		//$tbd[3]['BRD_TYPE'] = 'R';
		$pdf->tbDrawData($tbd);
		...
		$counter = 1;
		if (count($_data_skpd) > 1) {
			foreach ($_data_skpd as $key => $value) {
				//Bagian datanya...
				$td[0]['TEXT'] = "";
				$td[1]['TEXT'] = $counter .".";
				$td[2]['TEXT'] = $value['rekening'];
				$td[3]['TEXT'] = $value['korek_nama']."\n\n";
				$td[4]['TEXT'] = format_currency($value['spt_pajak']);
				$td[5]['TEXT'] = "";
			...
			foreach ($_data_skpd as $key => $value) {
				if ($_data_skpd[0]['spt_jenis_pajakretribusi'] == '4' || $_data_skpd[0]['spt_jenis_pajakretribusi'] == '8') {
					//Bagian datanya...
					$td[0]['TEXT'] = "";
					$td[1]['TEXT'] = $counter .".";
					$td[2]['TEXT'] = $value['rekening'];
					if ($_data_skpd[0]['spt_jenis_pajakretribusi'] == '4') {
						$dt_rek = $model->get_spt_reklame($_data_skpd[0]['spt_dt_id']);
						$td[3]['TEXT'] = $value['korek_nama']."\n\n\n\n";
					} else {
						$td[3]['TEXT'] = $value['korek_nama']."\n\n\n\n\n\n\n";
					}
					...
					$td[1]['TEXT'] = $counter .".";
					$td[2]['TEXT'] = $value['rekening'];
					$td[3]['TEXT'] = $value['korek_nama']."\nDasar Pengenaan : Rp. ".format_currency($value['spt_dt_jumlah'])."\nPeriode : ".format_tgl($_data_skpd[0][spt_periode_jual1]).' s/d '.format_tgl($_data_skpd[0][spt_periode_jual2])."\n\n\n\n\n\n\n";
					$td[4]['TEXT'] = format_currency($value['spt_pajak']);
					$td[5]['TEXT'] = "";
		...
		//Bagian Jumlah...
		$total_pajak = $_data_skpd[0]['spt_pajak'];
		...
		$ket[0]['TEXT'] = "1.";$ket[1]['TEXT'] = "Harap penyetoran dilakukan melalui Bank Pembagunan Jawa Barat Banten (Bank BJB).";
		...
		$tnd[1]['TEXT'] = strtoupper($mengetahui->ref_pangpej_ket).', '.$mengetahui->ref_goru_ket;;
		$pdf->tbDrawData($tnd);
		...
		if ($this->session->userdata('USER_SPT_CODE') == "10" && $this->input->get('spt_jenis_pajakretribusi') == "4") 
			$pdf->MultiCell(190,3,$pdf->SetFont('Arial','',6).$pdf->Cell(15,2,'Lembar Merah','','','L').$pdf->Cell(3,2,':','','','C').$pdf->Cell(80,2,'DPMPST','','','L'),'','','',2);
		else 
			$pdf->MultiCell(190,3,$pdf->SetFont('Arial','',6).$pdf->Cell(15,2,'Lembar Merah','','','L').$pdf->Cell(3,2,':','','','C').$pdf->Cell(80,2,'Unit Pelaksana Teknis Badan Pendapatan','','','L'),'','','',2);
			
		$pdf->MultiCell(190,3,$pdf->SetFont('Arial','',6).$pdf->Cell(15,2,'Lembar Kuning','','','L').$pdf->Cell(3,2,':','','','C').$pdf->Cell(80,2,'Subid Evaluasi dan Pelaporan','','','L'),'','','',2);
		...
		$pdf->MultiCell(190,3,$pdf->SetFont('Arial','',6).$pdf->Cell(15,2,'Lembar Biru','','','L').$pdf->Cell(3,2,':','','','C').$pdf->Cell(80,2,'Subid Pendaftaran dan Penetapan Pajak Daerah','','','L'),'','','',2);
