...
$arrbulan = array(1=>'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember');
 ...
 // HTTP headers
HeaderingExcel('daftar_ketetapan_reklame.xls');
...
$worksheet1->write_string(0,0, "DAFTAR SKPD REKLAME", $ftitle); 
$worksheet1->merge_cells(0, 0, 0, 9);

$worksheet1->write_string(1,0, "TAHUN : " . $_GET['tahun'], $ftitle); 
$worksheet1->merge_cells(1, 0, 1, 9);

$worksheet1->write_string(2,0, "MASA PAJAK : ".strtoupper(getNamaBulan($_GET['bulan'])), $ftitle); 
...
$worksheet1->write_string($no_baris,1, "SKPD",$fheader);
$worksheet1->write_string($no_baris,2, "",$fheader);
$worksheet1->merge_cells($no_baris, 1, $no_baris, 2);
$worksheet1->write_string($no_baris,3, "NAMA WAJIB PAJAK",$fheader);
$worksheet1->merge_cells($no_baris, 3, $no_baris + 1, 3);
$worksheet1->write_string($no_baris,4, "NASKAH REKLAME",$fheader);
...
$worksheet1->write_string($no_baris,6, "LOKASI PASANG", $fheader);
$worksheet1->merge_cells($no_baris, 6, $no_baris + 1, 6);
$worksheet1->write_string($no_baris,7, "JENIS REKLAME", $fheader); 
$worksheet1->merge_cells($no_baris, 7, $no_baris + 1, 7);
$worksheet1->write_string($no_baris,8, "KETETAPAN", $fheader);
$worksheet1->merge_cells($no_baris, 8, $no_baris + 1, 8);
$worksheet1->write_string($no_baris,9, "JATUH TEMPO", $fheader);
...
if (!empty($data)) {
	foreach($data as $key => $val) {
		...
		$worksheet1->write($no_baris,4, $val['sptrek_judul'],$fdata);
		$worksheet1->write($no_baris,5, preg_replace("\r|\n","",$val['wp_wr_almt']),$fdata);
		if($val['wp_wr_lurah'] == null){
			$worksheet1->write($no_baris,6, $val['sptrek_lokasi'],$fdata);
		}else{
			$worksheet1->write($no_baris,6, $val['sptrek_lokasi'] . " Kel. " . $val['wp_wr_lurah'] . " Kec. " . $val['wp_wr_camat'],$fdata);
		}
		$worksheet1->write($no_baris,7, $val['korek_nama_detail'],$fdata);
		$worksheet1->write_number($no_baris,8, $val['spt_dt_pajak'],$fcurrency);
		$worksheet1->write_number($no_baris,9, ceil((strtotime($val['netapajrek_tgl_jatuh_tempo']) + $ut_to_ed_diff) / $seconds_in_a_day),$fdate);
...
$worksheet1->write_string($no_baris,6, "", $fheader);
$worksheet1->write_string($no_baris,7, "",$fheader);
$worksheet1->write_number($no_baris,8, $pajak, $ftotal_currency); 
$worksheet1->write_string($no_baris,9, "", $fheader);
