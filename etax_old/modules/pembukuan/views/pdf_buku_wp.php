...
require_once APPPATH.'libraries/Fungsi.php';
...
if (!empty($dt_rows)) {
    ...
    $pdf->Ln(0);
    $pdf->Cell(122,5,"",1,0,'C');
    $pdf->Cell(84,5,$title_column,1,0,'C');
    $pdf->Cell(84,5,"P E N Y E T O R A N",1,0,'C');
    $pdf->Cell(25,5,"",1,2,'C');
       
    $pdf->Ln(0);
    $pdf->Cell(6,5,"No.",1,0,'C');
    $pdf->Cell(24,5,"REKENING",1,0,'C');
    $pdf->Cell(42,5,"U R A I A N",1,0,'C');
	$pdf->Cell(50,5,"MASA PAJAK",1,0,'C');
    $pdf->Cell(17,5,$column_tgl,1,0,'C');
    $pdf->Cell(17,5,$column_kohir,1,0,'C');
    $pdf->Cell(25,5,$column_jumlah,1,0,'C');
    $pdf->Cell(25,5,$column_ketetapan,1,0,'C');
    $pdf->Cell(17,5,"NO. REG",1,0,'C');
    $pdf->Cell(17,5,"TGL REG",1,0,'C');
    $pdf->Cell(25,5,"DENDA",1,0,'C');
    $pdf->Cell(25,5,"JUMLAH SETOR",1,0,'C');
    $pdf->Cell(25,5," S I S A",1,2,'C');   

   
	$counter = 1;
    // $contain = array();
    // $n=count($ar_npwpd); $pdf->Ln(0);
    // $pdf->Cell(6,1,"",'LR',0,'C').$pdf->Cell(24,1,"",'R',0,'C').$pdf->Cell(42,1,"",'R',0,'C').$pdf->Cell(50,1,"",'R',0,'C').$pdf->Cell(17,1,"",'R',0,'C').$pdf->Cell(17,1,"",'R',0,'C').$pdf->Cell(25,1,"",'R',0,'C').$pdf->Cell(25,1,"",'R',0,'C').$pdf->Cell(17,1,"",'R',0,'C').$pdf->Cell(17,1,"",'R',0,'C').$pdf->Cell(25,1,"",'R',0,'C').$pdf->Cell(25,1,"",'R',2,'C');
    $pdf->Ln(0);
    $total_tetap = 0;
    $total_setor = 0;
    $total_sisa = 0;
	...
	if($_GET['jenis_pemungutan'] == 2) {
            if ($vw['setorpajret_jlh_bayar'] == null){
                $tgljatuhtempo = date('Y-m-d', strtotime('+1 month', strtotime($vw['spt_periode_jual1'])));
                if(date("m", strtotime($vw['spt_periode_jual1'])) == '01'){
                    $t_entry = date("Y-m-d", strtotime('+27 days', strtotime($tgljatuhtempo)));
                }else {
                    $t_entry = date("Y-m-d", strtotime('+29 days', strtotime($tgljatuhtempo)));
                } 
                $denda = denda($t_entry, date("Y-m-d"), $vw['spt_pajak']);
            }else{
                $tgljatuhtempo = date('Y-m-d', strtotime('+1 month', strtotime($vw['spt_periode_jual1'])));
                if(date("m", strtotime($vw['spt_periode_jual1'])) == '01'){
                    $t_entry = date("Y-m-d", strtotime('+27 days', strtotime($tgljatuhtempo)));
                ...
                if ($vw['setorpajret_tgl_bayar'] >= '2023-08-01' && $vw['setorpajret_tgl_bayar'] <= '2023-09-30'){
                    $denda = 0;
                }else{
                    $denda = denda($t_entry, $vw['setorpajret_tgl_bayar'], $vw['spt_pajak']);
                } 
            }
	        $sisa = ($vw['spt_pajak'] + $denda) - $vw['setorpajret_jlh_bayar'];
	        if ($sisa < 0)
	        	$sisa = 0; 
	        $total_sisa += $sisa;
            $total_denda += $denda;
    	
        	$pdf->MultiCell(310, 5, $pdf->Cell(6,5,$counter,'LR',0,'L').
        							$pdf->Cell(24,5,$vw['koderek'],'R',0,'L').
        							$pdf->Cell(42,5,$vw['korek_nama'],'R',0,'L').
									$pdf->Cell(50,5,format_tgl($vw['spt_periode_jual1'])."  s.d  ".format_tgl($vw['spt_periode_jual2']),'R',0,'C').
        							$pdf->Cell(17,5,format_tgl($vw['netapajrek_tgl']),'R',0,'C').
        							$pdf->Cell(17,5,$vw['netapajrek_kohir'],'R',0,'C').
        							$pdf->Cell(25,5,format_currency($vw['spt_pajak']),'R',0,'R').
        							$pdf->Cell(25,5,'SKPD','R',0,'C').
        							$pdf->Cell(17,5,$vw['setorpajret_no_bukti'],'R',0,'C').
        							$pdf->Cell(17,5,format_tgl($vw['setorpajret_tgl_bayar']),'R',0,'C').
                                    $pdf->Cell(25,5,format_currency($denda),'R',0,'R').
        							$pdf->Cell(25,5,format_currency($vw['setorpajret_jlh_bayar']),'R',0,'R').
        							$pdf->Cell(25,5,format_currency($sisa),'R',0,'R'),'','','L');
        }
        else {
            if ($vw['setorpajret_jlh_bayar'] == null){
                $tgljatuhtempo = date('Y-m-d', strtotime('+1 month', strtotime($vw['spt_periode_jual1'])));
            ...
            $pdf->MultiCell(310,5,$pdf->Cell(6,5,$counter,'LR',0,'L').
									...
									$pdf->Cell(50,5,format_tgl($vw['spt_periode_jual1'])."  s.d  ".format_tgl($vw['spt_periode_jual2']),'R',0,'C').
           							...
           							$pdf->Cell(25,5,format_currency($omset),'R',0,'R').
           							$pdf->Cell(25,5,format_currency($vw['spt_pajak']),'R',0,'R').
           							$pdf->Cell(17,5,$vw['setorpajret_no_bukti'],'R',0,'C').
           							$pdf->Cell(17,5,format_tgl($vw['setorpajret_tgl_bayar']),'R',0,'C').
                                    $pdf->Cell(25,5,format_currency($denda),'R',0,'R').
	...
	if($_GET['jenis_pemungutan'] == 2) {
        ...
        $pdf->MultiCell(310,5,$pdf->Cell(6+24+42+50,5,"T O T A L  S E L U R U H N Y A",'LTB',0,'C').$pdf->Cell(17+17+25,5,'','TB').$pdf->Cell(25,5,number_format ($total_tetap, 2,',','.'),'TB',0,'R').$pdf->Cell(17+17,5,"",'TB',0,'C').$pdf->Cell(25,5,number_format ($total_denda, 2,',','.'),'TB',0,'R').$pdf->Cell(25,5,number_format ($total_setor, 2,',','.'),'TB',0,'R').$pdf->Cell(25,5,number_format ($total_sisa, 2,',','.'),$b.'TRB',0,'R'),'','','L');
  	} else {
