...
class Daftar_rekapitulasi extends Master_Controller
{
	...
	function index()
	{
		$data['title'] = 'Cetak Daftar Rekapitulasi';
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pembukuan';
		$data['content_desc'] = 'Cetak Daftar Rekapitulasi';
		$page = 'pembukuan/form_daftar_rekapitulasi';
	...
	function cetak_daftar($type_cetak = 0, $lap_realisasi = 1)
	{
		...
		if ($_GET['status_spt'] == "8") {
			...
			$worksheet1->merge_cells(1, 0, 1, 13);

			$worksheet1->write_string(3, 0, "NO", $fheader);
			$worksheet1->merge_cells(3, 0, 4, 0);
			$worksheet1->write_string(3, 1, "WAJIB PAJAK", $fheader);
			$worksheet1->write_blank(3, 2, $fheader);
			$worksheet1->write_blank(3, 3, $fheader);
			$worksheet1->write_blank(3, 4, $fheader);
			$worksheet1->merge_cells(3, 1, 3, 4);
			$worksheet1->write_string(3, 5, "$ket_spt", $fheader);
			$worksheet1->write_blank(3, 6, $fheader);
			$worksheet1->write_blank(3, 7, $fheader);
			$worksheet1->write_blank(3, 8, $fheader);
			$worksheet1->merge_cells(3, 5, 3, 8);
			$worksheet1->write_string(3, 9, "STS", $fheader);
			$worksheet1->write_blank(3, 10, $fheader);
			$worksheet1->write_blank(3, 11, $fheader);
			$worksheet1->write_blank(3, 12, $fheader);
			$worksheet1->merge_cells(3, 9, 3, 12);
			$worksheet1->write_string(3, 13, "TUNGGAKAN (Rp)", $fheader);
			$worksheet1->merge_cells(3, 13, 4, 13);
			...
			$worksheet1->write_string(4, 11, "DENDA", $fheader);
			$worksheet1->write_string(4, 12, "SETORAN (Rp)", $fheader);
			$worksheet1->write_blank(4, 13, $fheader);
			...
			$worksheet1->write_number(5, 13, "14", $fheader_small);
		} elseif($_GET['jenis_pajak'] == "4") {
			$worksheet1->merge_cells(0, 0, 0, 16);
			$worksheet1->merge_cells(1, 0, 1, 16);
			...
			$worksheet1->write_string(3, 8, "URAIAN REKLAME", $fheader);
			$worksheet1->write_blank(3, 8, $fheader);
			$worksheet1->write_blank(3, 9, $fheader);
			$worksheet1->write_blank(3, 10, $fheader);
			$worksheet1->write_blank(3, 11, $fheader);
			$worksheet1->merge_cells(3, 8, 3, 11);
			$worksheet1->write_string(3, 12, "STS", $fheader);
			$worksheet1->write_blank(3, 12, $fheader);
			$worksheet1->write_blank(3, 13, $fheader);
			$worksheet1->write_blank(3, 14, $fheader);
			$worksheet1->write_blank(3, 15, $fheader);
			$worksheet1->merge_cells(3, 12, 3, 15);
			$worksheet1->write_string(3, 16, "TUNGGAKAN (Rp)", $fheader);
			$worksheet1->merge_cells(3, 16, 4, 16);
			...
			$worksheet1->write_string(4, 8, "Naskah", $fheader);
			$worksheet1->write_string(4, 9, "Lokasi Pasang", $fheader);
			$worksheet1->write_string(4, 10, "Ukuran", $fheader);
			$worksheet1->write_string(4, 11, "Tayang S.D", $fheader);
			$worksheet1->write_string(4, 12, "NOMOR", $fheader);
			$worksheet1->write_string(4, 13, "TANGGAL", $fheader);
			$worksheet1->write_string(4, 14, "DENDA", $fheader);
			$worksheet1->write_string(4, 15, "SETORAN (Rp)", $fheader);
			$worksheet1->write_blank(4, 16, $fheader);
			...
			$worksheet1->write_number(5, 12, "13", $fheader_small);
			$worksheet1->write_number(5, 13, "14", $fheader_small);
			$worksheet1->write_number(5, 14, "15", $fheader_small);
			$worksheet1->write_number(5, 15, "16", $fheader_small);
			$worksheet1->write_number(5, 16, "17", $fheader_small);
		}else{
			...
			$worksheet1->write_blank(3, 8, $fheader);$worksheet1->write_blank(3, 9, $fheader);$worksheet1->write_blank(3, 10, $fheader);$worksheet1->write_blank(3, 11, $fheader);
			$worksheet1->merge_cells(3, 8, 3, 11);
			...
			$worksheet1->write_string(4, 10, "DENDA", $fheader);
		...
		if ($_GET['jenis_pajak'] != "4") {
			...
			if (count($dt_kecamatan) > 0) {
				foreach ($dt_kecamatan as $camat) {
					...
					//get data wp
					if ($type_cetak == 1) {
						$dt_wp = $this->daftar_rekapitulasi_model->get_wp_kecamatan(
							$camat['camat_id'],
						...
						if ($lap_realisasi == 1) {
							$dt_rekap = $this->daftar_rekapitulasi_model->get_data_rekapitulasi_realisasi(
								$lap_realisasi,
								$this->input->get('jenis_pajak'),
								$camat['camat_id'],
						...
						if ($_GET['status_spt'] == "8") {
							foreach ($dt_rekap as $row) {
								if($lap_realisasi != 1){
									$cek_bayar = $this->daftar_rekapitulasi_model->cek_bayar($row['spt_id']);
									if($cek_bayar == null){
										$cek_bayar = $this->daftar_rekapitulasi_model->cek_bayar_bank($row['spt_kode_billing'], $row['spt_periode']);
									}
								}
								...
								if($lap_realisasi !=1){
									$worksheet1->write_string($no_baris, 9, $cek_bayar->skbh_no, $fdata_center);
								}else{
								...
								if ($lap_realisasi != 1) {
									if ($cek_bayar->skbh_tgl != NULL)
										$worksheet1->write_number($no_baris, 10, ceil((strtotime($cek_bayar->skbh_tgl) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
									else
								...
								if ($lap_realisasi != 1) {
									if($cek_bayar->setorpajret_jlh_bayar != NULL)
										$worksheet1->write_formula($no_baris, 11, "=M$baris-I$baris", $fsisa);
									else
										$worksheet1->write_blank($no_baris, 11, $fdata);
								}else{
									if($row['setorpajret_jlh_bayar'] != NULL)
										$worksheet1->write_formula($no_baris, 11, "=M$baris-I$baris", $fsisa);
									else
										$worksheet1->write_blank($no_baris, 11, $fdata);
								}
								...
								if ($lap_realisasi != 1) {
									if ($cek_bayar->skbh_tgl != NULL)
										$worksheet1->write_number($no_baris, 12, $cek_bayar->setorpajret_jlh_bayar, $fcurrency);
									else
										$worksheet1->write_blank($no_baris, 12, $fdata);
								}else{
									if ($row['skbh_tgl'] != NULL)
										$worksheet1->write_number($no_baris, 12, $row['setorpajret_jlh_bayar'], $fcurrency);
									else
										$worksheet1->write_blank($no_baris, 12, $fdata);
								}

								$worksheet1->write_formula($no_baris, 13, "=(I$baris+L$baris)-M$baris", $fsisa);
							...
							foreach ($dt_rekap as $row) {
								...
								if($row['setorpajret_jlh_bayar'] != NULL)
									$worksheet1->write_formula($no_baris, 10, "=L$baris-H$baris", $fsisa);
								else
									$worksheet1->write_blank($no_baris, 10, $fdata);

								if ($row['skbh_tgl'] != NULL)
									$worksheet1->write_number($no_baris, 11, $row['setorpajret_jlh_bayar'], $fcurrency);
								else
									$worksheet1->write_blank($no_baris, 11, $fdata);

								$worksheet1->write_formula($no_baris, 12, "=(H$baris+K$baris)-L$baris", $fsisa);
				...
				if ($_GET['status_spt'] == "8") {
					...
					$worksheet1->write_blank($no_baris, 1, $ftotal);
					$worksheet1->write_blank($no_baris, 2, $ftotal);
					$worksheet1->write_blank($no_baris, 3, $ftotal);
					$worksheet1->write_blank($no_baris, 4, $ftotal);
					$worksheet1->write_blank($no_baris, 5, $ftotal);
					$worksheet1->write_blank($no_baris, 6, $ftotal);
					...
					$worksheet1->write_blank($no_baris, 9, $ftotal);
					$worksheet1->write_blank($no_baris, 10, $ftotal);
					$worksheet1->write_blank($no_baris, 11, $ftotal);
					$worksheet1->merge_cells($no_baris, 9, $no_baris, 11);
					$worksheet1->write_formula($no_baris, 12, "=SUM(M7:M$no_baris)", $ftotal_currency);
					$worksheet1->write_formula($no_baris, 13, "=SUM(N7:N$no_baris)", $ftotal_sisa);
				} else {
					//bagian total
					$worksheet1->write_string($no_baris, 0, "TOTAL", $ftotal);
					$worksheet1->write_blank($no_baris, 1, $ftotal);
					...
					$worksheet1->merge_cells($no_baris, 0, $no_baris, 6);
					$worksheet1->write_formula($no_baris, 7, "=SUM(H7:H$no_baris)", $ftotal_currency);
					$worksheet1->write_blank($no_baris, 8, $ftotal);
					$worksheet1->write_blank($no_baris, 9, $ftotal);
					$worksheet1->write_blank($no_baris, 10, $ftotal);
			...
			if($this->input->get('camat_id') != ""){
				$getCamat = $this->daftar_rekapitulasi_model->getCamat($this->input->get('camat_id'));
				if ($no_baris > 6){
					$no_baris++;
				}
				$worksheet1->write_blank($no_baris, 0, $fkecamatan);
				$worksheet1->write_string($no_baris, 1, $getCamat->camat_nama, $fkecamatan);
				$worksheet1->write_blank($no_baris, 2, $fkecamatan);
				if ($_GET['status_spt'] == "8") {
					for ($i = 3; $i <= 11; $i++)
						$worksheet1->write_blank($no_baris, $i, $fkecamatan);
				...
				foreach ($dt_rekap as $row) {
					...
					$lama_pasang = $row['sptrek_lama_pasang'];
					$tgl_kadaluarsa = date('Y-m-d', strtotime(''.+$lama_pasang.' days', strtotime($row['spt_tgl_proses']))); // penjumlahan tanggal sebanyak 7 hari
					...
					$worksheet1->write_string($no_baris, 6, $row['spt_nomor'], $fdata_center);
					$worksheet1->write_number($no_baris, 7, $row['spt_pajak'], $fcurrency);
					$worksheet1->write_string($no_baris, 8, $row['sptrek_judul'], $fdata);
					$worksheet1->write_string($no_baris, 9, $row['sptrek_lokasi'], $fdata);
					$worksheet1->write_string($no_baris, 10, $row['sptrek_luas'], $fdata);
					$worksheet1->write_number($no_baris, 11, ceil((strtotime($tgl_kadaluarsa) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
					// $worksheet1->write_string($no_baris, 12, $row['skbh_no'], $fdata_center);
					if($lap_realisasi !=1){
						$worksheet1->write_string($no_baris, 12, $cek_bayar->skbh_no, $fdata_center);
					}else{
						$worksheet1->write_string($no_baris, 12, $row['skbh_no'], $fdata_center);
					}
					...
					// if ($row['skbh_tgl'] != NULL)
					// 	$worksheet1->write_number($no_baris, 13, ceil((strtotime($row['skbh_tgl']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
					// else
					// 	$worksheet1->write_blank($no_baris, 13, $fdate);
					if ($lap_realisasi != 1) {
						if ($cek_bayar->skbh_tgl != NULL)
							$worksheet1->write_number($no_baris, 13, ceil((strtotime($cek_bayar->skbh_tgl) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
						else
							$worksheet1->write_blank($no_baris, 13, $fdate);
					}else{
						if ($row['skbh_tgl'] != NULL)
					...
					// if($row['skbh_jumlah'] != NULL)
					//     $worksheet1->write_formula($no_baris, 14, "=P$baris-H$baris", $fsisa);
					// else
					...
					// if ($row['skbh_tgl'] != NULL)
					// 	$worksheet1->write_number($no_baris, 15, $row['skbh_jumlah'], $fcurrency);
					// else
					// 	$worksheet1->write_blank($no_baris, 15, $fdata);
					if ($lap_realisasi != 1) {
						if ($cek_bayar->skbh_tgl != NULL)
							$worksheet1->write_number($no_baris, 15, $cek_bayar->setorpajret_jlh_bayar, $fcurrency);
						else
							$worksheet1->write_blank($no_baris, 15, $fdata);
					}else{
						if ($row['skbh_tgl'] != NULL)
							$worksheet1->write_number($no_baris, 15, $row['setorpajret_jlh_bayar'], $fcurrency);
						else
							$worksheet1->write_blank($no_baris, 15, $fdata);
					}

					$worksheet1->write_formula($no_baris, 16, "=(H$baris+O$baris)-P$baris", $fsisa);
				...
				$worksheet1->write_blank($no_baris, 6, $ftotal);
				$worksheet1->merge_cells($no_baris, 0, $no_baris, 6);
				$worksheet1->write_formula($no_baris, 7, "=SUM(H7:H$no_baris)", $ftotal_sisa);
				$worksheet1->write_blank($no_baris, 8, $ftotal);
				$worksheet1->write_blank($no_baris, 9, $ftotal);
				$worksheet1->write_blank($no_baris, 10, $ftotal);
				$worksheet1->write_blank($no_baris, 11, $ftotal);
				$worksheet1->write_blank($no_baris, 12, $ftotal);
				$worksheet1->write_blank($no_baris, 13, $ftotal);
				$worksheet1->write_blank($no_baris, 14, $ftotal);
				$worksheet1->merge_cells($no_baris, 8, $no_baris, 14);
				$worksheet1->write_formula($no_baris, 15, "=SUM(P7:P$no_baris)", $ftotal_sisa);
				$worksheet1->write_formula($no_baris, 16, "=SUM(Q7:Q$no_baris)", $ftotal_sisa);
			}else {
				//load data kecamatan
				$dt_kecamatan = $this->daftar_rekapitulasi_model->get_kecamatan(
					$lap_realisasi,
				...
				if (count($dt_kecamatan) > 0) {
					foreach ($dt_kecamatan as $camat) {
						...
						$worksheet1->write_blank($no_baris, 0, $fkecamatan);
						if ($camat['camat_nama'] == ''){
							$worksheet1->write_string($no_baris, 1, 'Luar Kota', $fkecamatan);
						}else{
							$worksheet1->write_string($no_baris, 1, $camat['camat_nama'], $fkecamatan);
						}
						$worksheet1->write_blank($no_baris, 2, $fkecamatan);
	...
	function cetak_pendataan($type_cetak = 0, $pendataan = 1)
	{
		...
		// HTTP headers
		HeaderingExcel('daftar-rekapitulasi_pendataan.xls');
		...
		if ($pendataan == 1) {
			$worksheet1->write_string(1, 0, "Berdasarkan Tanggal Pendataan " . format_tgl($this->input->get('awDate'), true, true, false) . " - " .
		...
		if ($_GET['jenis_pajak'] != "4") {
			...
			if (count($dt_kecamatan) > 0) {
				...
				foreach ($dt_kecamatan as $camat) {
					...
					//get data wp
					if ($type_cetak == 1) {
						$dt_wp = $this->daftar_rekapitulasi_pendataan_model->get_wp_kecamatan(
							$camat['camat_id'],
							"0" . $this->input->get('jenis_pajak'),
							$this->input->get('jenis_restoran'),
							$bulan_pajak,
						...
						if ($_GET['status_spt'] == "8") {
							...
							foreach ($dt_rekap as $row) {
								...
								$worksheet1->write_string($no_baris, 2, $row['npwprd'], $fdata_center);
								$worksheet1->write_string($no_baris, 3, strtoupper($row['wp_wr_almt']), $fdata);
								$worksheet1->write_string($no_baris, 4, date('F, Y', strtotime($row['spt_periode_jual1'])), $fdata_center);
								$worksheet1->write_number($no_baris, 5, ceil((strtotime($row['spt_tgl_proses']) + $ut_to_ed_diff) / $seconds_in_a_day), $fdate);
								$worksheet1->write_string($no_baris, 6, $row['spt_nomor'], $fdata_center);
								$worksheet1->write_number($no_baris, 7, $row['omzet'], $fcurrency);
								$worksheet1->write_number($no_baris, 8, $row['spt_pajak'], $fcurrency);
								...
								$worksheet1->write_number($no_baris, 12, $tunggakan, $fsisa);
							...
							foreach ($dt_rekap as $row) {
								...
								$tunggakan = $row['setoran'] - $row['spt_pajak'];
								$worksheet1->write_number($no_baris, 11, $tunggakan, $fsisa);
				...
				if ($_GET['status_spt'] == "8") {
					...
					// $worksheet1->write_formula($no_baris, 7, "=SUM(H7:H".$no_baris.")", $ftotal_currency);
					$worksheet1->write_number($no_baris, 7, $tot_omzet, $ftotal_currency);
					...
					// $worksheet1->write_formula($no_baris, 11, "=SUM(L7:L".$no_baris.")", $ftotal_currency);
					$worksheet1->write_number($no_baris, 11, $tot_setoran, $ftotal_currency);

					// $worksheet1->write_formula($no_baris, 12, "=SUM(M7:M".$no_baris.")", $ftotal_sisa);
					$worksheet1->write_number($no_baris, 12, $tot_tunggakan, $ftotal_sisa);
				} else {
					//bagian total
					$worksheet1->write_string($no_baris, 0, "TOTAL", $ftotal);
					$worksheet1->write_blank($no_baris, 1, $ftotal);
					...
					$worksheet1->write_number($no_baris, 10, $tot_setoran, $ftotal_currency);

					// $worksheet1->write_formula($no_baris, 11, "=SUM(L7:L".$no_baris.")", $ftotal_sisa);
					$worksheet1->write_number($no_baris, 11, $tot_tunggakan, $ftotal_sisa);
				}
			}
		} else {
