...
class Buku_kendali extends Master_Controller
{
	...
	function index()
	{
		...
		$data['title'] = 'Pembukuan | Cetak Realisasi & Buku Kendali';
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pembukuan';
		$data['content_desc'] = 'Buku Kendali';
		$page = 'pembukuan/form_buku_kendali';
	...
	function cetak_realisasi()
	{
		...
		$this->common_model->history_log(
			"pembukuan",
			"P",
			"Cetak realisasi : " . $_GET['jenis_pajak_realisasi'] . " |" . $_GET['jenis_pajak_restoran'] . " |" . $_GET['f_bulan_realisasi'] . " - " . $_GET['f_tahun_realisasi'] . " | " . $_GET['t_bulan_realisasi'] . " - " . $_GET['t_tahun_realisasi']
		);
		...
		$diff_realisasi = get_diff_months(
			$f_tahun_realisasi . "-" . ((strlen($f_bulan_realisasi) == 1) ? "0" . $f_bulan_realisasi : $f_bulan_realisasi) . "-01",
		...
		$pjk = $this->input->get('jenis_pajak_realisasi');
		$res = $this->input->get('jenis_pajak_restoran');
		if ($res == 0 and $pjk == 2) {
			$resto = 'RUMAH MAKAN DAN KATERING';
		} else if ($res == 1 and $pjk == 2) {
			$resto = 'RUMAH MAKAN';
		} else if ($res == 2 and $pjk == 2) {
			$resto = 'KATERING';
		} else {
			$resto = '';
		}
		...
		$worksheet1->write_string(0, 0, "REALISASI " . strtoupper($this->buku_kendali_model->get_ref_pajak($this->input->get('jenis_pajak_realisasi')) . " " . $resto), $ftitle);
		...
		$worksheet1->write_string(2, 3, "Jumlah", $fheader);
		...
		$no_baris = 4;
		$dt_realisasi = $this->buku_kendali_model->get_data_realisasi(
			$this->input->get('jenis_pajak_realisasi'),
			$this->input->get('jenis_pajak_restoran'),
			$f_bulan_realisasi,
			$f_tahun_realisasi,
			$t_bulan_realisasi,
			$t_tahun_realisasi,
	...
	function cetak_kendali()
	{
		...
		$this->common_model->history_log(
			"pembukuan",
			"P",
			"Cetak buku kendali : " . $_GET['jenis_pajak_kendali'] . " | " . $_GET['jenis_pajak_restoran2'] . " | " . $_GET['f_bulan_kendali'] . " - " . $_GET['f_tahun_kendali'] . " | " . $_GET['t_bulan_kendali'] . " - " . $_GET['t_tahun_kendali']
		);
		...
		$spt_ket = $this->input->get('status_spt_kendali');
		$jenis_pajak_restoran = $this->input->get('jenis_pajak_restoran');
		...
		$diff_masa_pajak = get_diff_months(
			$f_tahun . "-" . ((strlen($f_bulan) == 1) ? "0" . $f_bulan : $f_bulan) . "-01",
		...
		$pjk = $this->input->get('jenis_pajak_kendali');
		$res = $this->input->get('jenis_pajak_restoran2');
		if ($res == 0 and $pjk == 2) {
			$resto = 'RUMAH MAKAN DAN KATERING';
		} else if ($res == 1 and $pjk == 2) {
			$resto = 'RUMAH MAKAN';
		...
		$worksheet2->write_string(0, 0, "BUKU KENDALI " . strtoupper($this->buku_kendali_model->get_ref_pajak($this->input->get('jenis_pajak_kendali'))) . " " . $resto . " " . $sptkk, $ftitle);
		...
		$no_baris = 3;
		$dt_penerimaan = $this->buku_kendali_model->get_data_pajak(
			$this->input->get('jenis_pajak_kendali'),
			$this->input->get('jenis_pajak_restoran2'),
			$f_bulan,
			$f_tahun,
			$t_bulan,
			$t_tahun,
			$this->input->get('denda_kendali')
		);
