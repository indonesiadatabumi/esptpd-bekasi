	...
	for (k=0; k<=i; k++) {
		try {
			if ($('#setorpajret_id'+k).val() == $('#setorpajret_id'+id).val() && k != id) {
				showWarning('Nomor SSPD yang sama sudah terpilih. Mohon pilih nomor lainnya!');
...
var calcTotal = function() {
	...
	$('#txt_total_setor').val(formatCurrency(totalPajak));
};
...
var createEventToolbar = function() {
	var submitForm = function() {
		...
		if (i > 0) {
			...
			showWarning("Silahkan masukkan data SSPD terlebih dahulu.");
			$('#btn_save').one("click", submitForm);
		}
	};
	...
	$("#btn_print").click(function() {
		url = GLOBAL_SETOR_BANK_VARS["print"] + "?skbh_id=" + $("input[name=setor_bank_id]").val();
	...
	$("#btn_reset").click(function() {
		...
		$("#txt_alamat").val("");
		$("#txt_keterangan").val("");
		$("#ddl_bukti_setoran").val("1");
		$("tr.row").parent().remove();
		$("#btn_save").show();
		$("#btn_print").hide();
		$("input[name=setor_bank_id]").val("");
		calcTotal();
	});
};
