...
var insertNomorBukti = function(idSelect) {
	$.get(GLOBAL_FORM_CETAK_STS_VARS["get_penyetoran"], { tgl_penyetoran : $("#tgl_penyetoran").val(), spt_jenis_pajakretribusi : $("#spt_jenis_pajakretribusi").val() }, 
			function(data){
		var option = "";
...
var completePage = function() {	
	...
	$("#ddl_dari").change(function() {
		try {
			if ($("#ddl_dari :selected").val() != "-") {
				var arr_uptd = $("#ddl_dari :selected").val().split("|");
				
				$('#txt_dari').val(arr_uptd[1]);
				$('#txt_alamat').val(arr_uptd[2]);
				
				$('#txt_dari, #txt_alamat').attr("readonly", "readonly");
			} else {
				$('#txt_dari, #txt_alamat').removeAttr("readonly");
				
				$('#txt_dari, #txt_alamat').val("");
				$('#txt_dari').focus();
			}			
		} catch (err) {
...
var createEventToolbar = function() {
	var submitForm = function() {
		var showInsertResponse = function (response, status) {
            if(response.status == true) {
            	$("input[name=setor_bank_id]").val(response.id);
            	$("#btn_print").slideDown("slow");
            	$("#btn_save").hide();            	
            	showNotification("Data berhasil disimpan.");
            	
            	if ($("input[name=is_cart]").val() == "1")
            		$.post(GLOBAL_FORM_CETAK_STS_VARS["empty_cart"]);
            } else {
		...
		var save_options = { 
			url : GLOBAL_FORM_CETAK_STS_VARS["insert"],
			type : "POST",
			dataType: 'json',
	...
	$("#btn_print").click(function() {
		url = GLOBAL_FORM_CETAK_STS_VARS["print"] + "?skbh_id=" + $("input[name=setor_bank_id]").val();
	...
	$("#btn_back").click(function() {
		//load content panel
		load_content(GLOBAL_FORM_CETAK_STS_VARS["back"]);
	});
};
...
var focusInput = function() {
	...
	$("input[type=text],input[type=radio],input[type=checkbox],input[type=button],select").keypress(function(e) {
		...
		if ((e.which==13 || e.which==9) && isShift == false) {
			if (this.name == "txt_dari" ) { $('input[name=txt_alamat]').focus();}
			else if (this.name == "txt_alamat" ) $('input[name=txt_keterangan]').focus();
