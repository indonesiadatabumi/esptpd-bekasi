...
class Rekam_pajak extends Master_Controller
{
	...
	function cetak_sspd()
	{
		...
		$data['keterangan_spt'] = $this->common_model->get_record_list(
			"ketspt_id as value, '[' || ketspt_kode || '] ' ||ketspt_ket as item",
			"keterangan_spt",
			"ketspt_id IS NOT NULL",
			"ketspt_kode ASC"
		);

		$data['title'] = 'Pembayaran | Cetak SSPD';
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pembayaran';
		$data['content_desc'] = '';
		$page = 'bkp/form_cetak_sspd';
	...
	function setor_pajak()
	{
		...
		$keterangan_spt = $this->common_model->get_record_list(
			"ketspt_id as value, ketspt_singkat || ' - ' ||ketspt_ket as item",
			"keterangan_spt",
			"ketspt_id IS NOT NULL",
			"ketspt_kode ASC"
		);
		...
		$data['title'] = 'Pembayaran | Form Surat Ketetapan';
		$data['title_icon'] = 'pe-7s-news-paper';
		$data['content'] = 'Pembayaran';
		$data['content_desc'] = '';
		$page = 'bkp/form_rekam_setoran_pajak';
		echo modules::run('layouts/loadview', $data, $page);
	...
	function proses_setoran()
	{
		...
		if (count($arr_sspd) > 0) {
			if ($arr_sspd[0]['tgl_jatuh_tempo'] != null && $_POST['tanggal_setor'] != "") {
				...
				if ($diff_month > 25)
					$denda = ceil(0.02 * $arr_sspd[0]['spt_pajak'] * 24);
	 ...
	 * view setoran bank
	 */
	function view_setoran_bank()
	{
		$this->load->view('view_setoran_pajak_bank');
	}
	...
	/**
	 * get list setoran pajak bank
	 */
	function get_list_setoran_bank()
	{
		$this->load->model('setor_pajak_model');
		$this->setor_pajak_model->get_list_bank();
	}
