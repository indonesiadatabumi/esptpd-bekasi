...
if (!empty($arr_data)) {
	...
	$masa_pajak = $val['setorpajret_periode_jual1'];
		...
		if ($val['korek_jenis'] != "4") {
			...
			if($val['setorpajret_jenis_pajakretribusi'] == '4'){
				$data2[3]['TEXT'] = $val['wp_wr_nama']." - ".$arr_data_reklame[0]['sptrek_judul']."   -   ".getNamaBulan($arr_masa_pajak[1])." ".$arr_masa_pajak[0].$newline;
			}else{
				$data2[3]['TEXT'] = $val['wp_wr_nama']." - ".$val['sptrek_judul']."   -   ".getNamaBulan($arr_masa_pajak[1])." ".$arr_masa_pajak[0].$newline;
			}
		} else {
			$data2[3]['TEXT'] = $val['korek_nama'].$newline;
		}
	//	 			$rinc=$val['setorpajret_dt_jumlah']*0.15;
					
	//	$data2[4]['TEXT'] = format_currency($val['setorpajret_dt_jumlah']-$rinc);
	...
	//	$jumlah += $val['setorpajret_dt_jumlah']-$rinc;
	$jumlah = $val['skbh_jumlah'];
		$counter ++;
//	}
