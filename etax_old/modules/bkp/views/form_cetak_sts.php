						...
						<tbody>
							...
							if ($arr_data->num_rows() > 0) {
								foreach ($arr_data->result() as $row) {
									...
									$total_air_reklame = $row->setorpajret_jlh_bayar;
							...
							?>
									<tr>
										...
										if (
											$row->koderek == '4.1.1.08.01' || $row->koderek == '4.1.1.04.01' || $row->koderek == '4.1.1.04.02' || $row->koderek == '4.1.1.04.03' || $row->koderek == '4.1.1.04.04' || $row->koderek == '4.1.1.04.05'
											|| $row->koderek == '4.1.1.04.06' || $row->koderek == '4.1.1.04.07' || $row->koderek == '4.1.1.04.08' || $row->koderek == '4.1.1.04.09' || $row->koderek == '4.1.1.04.10' || $row->koderek == '4.1.1.04.12'
										) {


										?>

											<td align="right"><?= format_currency($total_air_reklame); ?></td>
										<?php
										} else {
										?>
							...
							<tr>
								...
								?>
									...
									<td align="right">
										<input type="text" name="txt_total_setor" id="txt_total_setor" value="<?= format_currency($total_air_reklame); ?>" class="inputbox" readonly="true" style="font-weight: bold; font-size: 14px; color: #18F518; background-color: black; text-align: right;" />
									</td>
								<?php } else {
								?>
									<td align="right">
										<input type="text" name="txt_total_setor" id="txt_total_setor" value="<?= format_currency($total); ?>" class="inputbox" readonly="true" style="font-weight: bold; font-size: 14px; color: #18F518; background-color: black; text-align: right;" />
									</td>
								<?php }	?>
							</tr>
						</tbody>
...
<script type="text/javascript">
	...
	GLOBAL_FORM_CETAK_STS_VARS["get_penyetoran"] = "bkp/setor_bank/get_nomor_bukti";
...
<script type="text/javascript" src="<?= base_url(); ?>modules/bkp/scripts/form_cetak_sts.js"></script>