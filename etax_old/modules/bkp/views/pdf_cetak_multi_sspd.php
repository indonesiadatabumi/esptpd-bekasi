...
if (!empty($arr_data)) {
	foreach ($arr_data as $key => $val) {
		...
		if ($ar_rekam_setor[0]['spt_jenis_pajakretribusi'] != "'")
			...
			$alamat = strtoupper($ar_rekam_setor[0]['wp_wr_almt']). strtoupper($wp_wr_lurah." ".$wp_wr_camat);
		else 
			// $alamat = strtoupper(ereg_replace("\r|\n"," ",$ar_rekam_setor[0]['wp_wr_almt']));
			$alamat = strtoupper($ar_rekam_setor[0]['wp_wr_almt']);
		//baris ke-2
		$r1[0]['TEXT'] = "";
		$r1[1]['TEXT'] = "ALAMAT";
		$r1[2]['TEXT'] = ":";
		...
		$pdf->MultiCell(190,3,$pdf->SetFont('Arial','',7).$pdf->Cell(25,2,'Lembar Biru','','','L').$pdf->Cell(3,2,':','','','C').$pdf->Cell(80,2,'Seksi Pendapatan Asli Daerah (PAD)','','','L'),'','','',2);
