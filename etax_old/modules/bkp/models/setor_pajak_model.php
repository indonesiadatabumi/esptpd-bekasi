...
class Setor_pajak_model extends CI_Model {
	...
	/**
	 * get list setoran bank
	 */
	function get_list_bank() {
		$tables = " v_payment_bank ";
	
		$sortname = $_POST['sortname'];
		$sortorder = $_POST['sortorder'];
		if (!$sortname) $sortname = ' tgl_pembayaran ';
		if (!$sortorder) $sortorder = 'desc';
		$sort = "ORDER BY $sortname $sortorder";
		...
		// echo $sql;
		$result = $this->adodb->Execute($sql);
		$total = $this->adodb->GetOne("SELECT count(DISTINCT tgl_pembayaran) FROM $tables $where");
			...
			while ($row = $result->FetchNextObject(false)) {
				$list[] = array (
								...
								"cell"	=> array (
									...
									$row->tgl_pembayaran,
									$row->ref_jenparet_ket,
									format_currency($row->sptpd_yg_dibayar),
									$row->npwprd,
									$row->wp_wr_nama,
								)
							);
	...
	function insert_setor_pajak() {
		...
		if ($this->is_setor_exist($_POST['spt_pen_id'], $_POST['setorpajret_jenis_ketetapan'])) {
			...
			//	if ( $_POST['korek_reklame_air']=='370' || $_POST['korek_reklame_air']=='247'){
				if ( $_POST['korek_reklame_air']=='4.1.1.08.01' || $_POST['korek_reklame_air']=='4.1.1.04.01' || $_POST['korek_reklame_air']=='4.1.1.04.02' || $_POST['korek_reklame_air']=='4.1.1.04.03' || $_POST['korek_reklame_air']=='4.1.1.04.04' || $_POST['korek_reklame_air']=='4.1.1.04.05'
				|| $_POST['korek_reklame_air']=='4.1.1.04.06' || $_POST['korek_reklame_air']=='4.1.1.04.07' || $_POST['korek_reklame_air']=='4.1.1.04.08' || $_POST['korek_reklame_air']=='4.1.1.04.09' || $_POST['korek_reklame_air']=='4.1.1.04.10' || $_POST['korek_reklame_air']=='4.1.1.04.12'){
					...
					if (!empty($_POST['spt_dt_korek'])) {
						foreach ($_POST['spt_dt_korek'] as $key => $value) {
							...
							$arr_setor_detail = array(
									...
									'setorpajret_dt_jumlah' => unformat_currency($_POST['txt_setoran'])
								);
