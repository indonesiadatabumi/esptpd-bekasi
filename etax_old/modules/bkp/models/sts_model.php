...
class Sts_model extends CI_Model {
	...
	function get_list($f_date = NULL, $t_date = NULL) {
		$tables = " setoran_ke_bank_header AS a";
		$join = " LEFT JOIN setoran_ke_bank_detail as b ON a.skbh_id=b.skbd_id_header
				LEFT JOIN setoran_pajak_retribusi AS c ON b.skbd_id_setoran_pajak=setorpajret_id
				LEFT JOIN wp_wr AS d ON c.setorpajret_id_wp=d.wp_wr_id";
		...
		$sql = "SELECT a.*, d.wp_wr_nama, d.wp_wr_almt
				FROM $tables $join $where $sort $limit";
	...
	function get_setoran_ke_bank_reklame($id) {
		$sql = "select * 
				from v_sts inner join spt on v_sts.setorpajret_no_spt=spt.spt_nomor 
				inner join spt_reklame on spt.spt_id=spt_reklame.sptrek_id_spt_dt 
				where skbh_id=$id
				ORDER BY koderek ASC";
		$arr_data = $this->adodb->GetAll($sql);
		return $arr_data;
