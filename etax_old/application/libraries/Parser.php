...
class Parser
{
    ...
    function _match($token)
    {
        switch ($token) {
            ...
            default:
                ...
                // if it's a reference
                if (
                    preg_match("/^[A-I]?[A-Z][0-9]+$/", $token) and
                ...
                elseif (
                    preg_match("/^[A-I]?[A-Z][0-9]+:[A-I]?[A-Z][0-9]+$/", $token) and
                ...
                elseif (preg_match("/^[A-Z0-9�-�\.]+$/", $token) and ($this->_lookahead == "(")) {
                    return ($token);
                }
                return '';
        }
    ...
    function _fact()
    {
        ...
        // if it's a range
        elseif (
            preg_match("/^[A-I]?[A-Z][0-9]+:[A-I]?[A-Z][0-9]+$/", $this->_current_token) or
        ...
        // if it's a function call
        elseif (preg_match("/^[A-Z0-9�-�\.]+$/", $this->_current_token)) {
            $result = $this->_func();
            return ($result);
        }
